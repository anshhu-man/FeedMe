...
const parsers = ['android-storage-smoke.mjs', 'verify-work-origin-planning.mjs'].map(name => {
  const source = fs.readFileSync(fileURLToPath(new URL(name, import.meta.url)), 'utf8');
  const declaration = source.match(/function workOriginSyncFailureEvidence\(log, identities\) \{[\s\S]*?\n\}/)?.[0];
  assert.ok(declaration, `${name} must retain the pure work-origin evidence parser`);
  return vm.runInNewContext(`(${declaration})`, Object.create(null), {timeout: 1000});
});

const testClass = 'com.feedme.storage.AndroidSessionWorkOriginPlanSyncFailureTest';
const expected = [
...
const lines = expected.map(([label, , sqlite]) =>
  `INSTRUMENTATION_RESULT: work_origin_sync_${label}=sqlite=${sqlite};failures=1;effects=0;vfs=1;delegatedUnlink=${sqlite === 1290 ? 1 : 0}`);
const log = lines.join('\n');
const rejects = (text, successful = identities) => {
  for (const parse of parsers) assert.throws(() => parse(text, successful));
};

test('work-origin smoke and verifier agree on all six source-bound select and replay VFS results', () => {
  const source = fs.readFileSync(fileURLToPath(new URL(
    '../shared/storage/src/androidInstrumentedTest/kotlin/com/feedme/storage/AndroidSessionWorkOriginPlanSyncFailureTest.kt',
...
test('work-origin evidence requires both selection and replay labels for every sync stage', () => {
  for (let omitted = 0; omitted < lines.length; omitted++)
    rejects(lines.filter((_, index) => index !== omitted).join('\n'));
});

test('work-origin evidence rejects duplicate labels even when the total count is still six', () => {
  rejects([...lines.slice(0, -1), lines[0]].join('\n'));
  rejects(`${log}\n${lines[0]}`);
});

test('work-origin evidence rejects forged or additional result labels', () => {
  rejects(log.replace('select-journal', 'select-unrecognized'));
  rejects(`${log}\n${lines[0].replace('select-journal', 'unrecognized-recovery')}`);
});

test('work-origin result bytes cannot substitute for a missing paired-success source identity', () => {
...
test('work-origin evidence rejects successful identities borrowed from another class or method', () => {
  rejects(log, identities.map(identity => identity.replace('AndroidSessionWorkOriginPlanSyncFailureTest', 'AndroidSessionLedgerSyncFailureTest')));
  rejects(log, identities.map(identity => `${identity}Unrelated`));
});

test('work-origin evidence rejects an incorrect SQLite extended error for the exact stage', () => {
...
test('work-origin evidence requires one actual failure and zero downstream effects', () => {
...
test('work-origin evidence preserves the explicit VFS mechanism and delegated-unlink stage', () => {
...
test('work-origin evidence rejects truncated reordered or extra result fields', () => {
