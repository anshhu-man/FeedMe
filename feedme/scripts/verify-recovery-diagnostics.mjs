...
// Read-only recovery diagnostics of already-open resources; no native-factory repair authority.
// Requires existing JDK17, SDK36, local PostgreSQL binaries and a booted local emulator.
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const destination = path.join(root, 'docs/verification/recovery-diagnostics');
...
const report = {
  ...
  scope: 'Read-only recovery diagnostics of already-open stores, an exact-owner atomic private-record inspection, redacted advisory findings and lifecycle/retirement fences. Diagnostics neither initialize native factories nor enumerate owners, run garbage collection, decrypt credential payloads, contact a provider, repair or erase data. Empty metadata is not globally empty storage; coherent metadata still requires independent identity verification and does not prove usable credentials. Fresh shared/server/isolated-PostgreSQL/Node regressions, five Android library builds/lint, and isolated native storage, credential, cancellation and separate real-store/runtime diagnostic integration tests. Runtime verification in these tests is fake or synthetic, not actual provider proof. Native tests use real stores and OS cancellation but no UI. Only storage has two separate instrumentation-process stages; no process-hard-kill or power-loss proof. Credential factory support remains gated to API 27+ (API 26 NOT_CONFIGURED), while factory-open failures and orphan repair are outside diagnostic coverage. Native hardlink and notification branches are retained explicitly; platform denial is not existing-hardlink guard proof and permission-unavailable is not delivered-notification cancellation proof. No production timer receiver/worker or app scheduling/recovery wiring, guest merge, account-deletion workflow, physical-device, iOS or release claim.',
