...
private val legacyUploadCapabilityFields = setOf("uploadUrl", "uploadMethod", "uploadFields", "uploadExpiresAt")
private val uploadCapabilityFields = legacyUploadCapabilityFields + "uploadCapability"
...
internal class MediaHttpInput private constructor(
    ...
    suspend fun readBody(channel: ByteReadChannel, validator: ContractBodyValidator): JsonObject? {
        val exact = readBoundedHttpBody(channel, if (hasBody) MediaHttpConfiguration.MAX_REQUEST_BYTES else 0, contentLength, ::invalidMedia)
...
internal fun validateMediaReply(operation: String, reply: StoredReply, validator: ContractBodyValidator, maxBytes: Int): String {
    ...
    if (operation != "prepareMediaUpload") check(capability.isEmpty())
        ...
        check(body.getValue("status").jsonPrimitive.content == "awaitingUpload" &&
            BigDecimal(body.getValue("version").jsonPrimitive.content).compareTo(BigDecimal.ONE) == 0)
        // A recovered prepare may deliberately return its original durable core without minting
        // another bearer: a prior remote issuance can have an unknown delivery outcome.
        if (capability.isEmpty()) return text
        val supabase = capability == setOf("uploadCapability")
        check(supabase || capability == legacyUploadCapabilityFields)
        val projected = if (supabase) body.getValue("uploadCapability").jsonObject else body
        val url = URI(projected.getValue(if (supabase) "url" else "uploadUrl").jsonPrimitive.content)
        check(url.scheme == "https" && !url.host.isNullOrBlank() && url.userInfo == null && url.fragment == null)
        if (supabase) {
            check(projected.getValue("protocol").jsonPrimitive.content == com.feedme.server.media.SUPABASE_MEDIA_PROTOCOL)
            check(projected.getValue("method").jsonPrimitive.content == "PUT")
        } else check(projected.getValue("uploadMethod").jsonPrimitive.content == "POST")
        ...
        Instant.parse(projected.getValue(if (supabase) "expiresAt" else "uploadExpiresAt").jsonPrimitive.content)
    }
    return text
}
