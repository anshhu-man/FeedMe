...
/** Symmetric draft-purpose composition/lease/operation admission over explicitly SYNTHETIC
 * trusted mapping and deny-I/O ports. Existing publication tests remain unchanged. These are
 * not a native verifier, live reviewed-Save token, bootstrap, or queue/caller-ACK proof. */
@OptIn(ExperimentalCoroutinesApi::class)
class ReviewedDraftPrincipalTest {
    ...
    @Test fun onlyTheActualDraftBorrowerCanOwnReviewedSaveAdmission() = runTest {
        val f = Fixture(StandardTestDispatcher(testScheduler)); val other = Fixture(StandardTestDispatcher(testScheduler))
        try {
            for (feature in listOf(MealKitchenFeature.POST_PUBLICATIONS, MealKitchenFeature.COOKING, MealKitchenFeature.COOKBOOK)) {
                val sibling = f.bind(feature)
                assertEquals(FailureReason.NOT_CONFIGURED, assertFailsWith<MealFailure> {
                    ReviewedDraftPrincipalAdmission(f.composition, sibling, f.source)
                }.reason)
            }
            assertEquals(FailureReason.STALE_SESSION, assertFailsWith<MealFailure> {
                ReviewedDraftPrincipalAdmission(f.composition, other.borrower, f.source)
            }.reason)
    ...
    @Test fun wrongIntegrationAndAnotherAdmissionBindingCannotSupplySnapshots() = runTest {
        ...
        try {
            ...
            val sameAccessDifferentAdmission = ReviewedDraftPrincipalAdmission(f.composition, f.borrower, f.source)
    ...
    @Test fun draftOwnerReleaseDuringSuspendedResolveKeepsPublicationSiblingUsable() = runTest {
        val f = Fixture(StandardTestDispatcher(testScheduler)); val sibling = f.bind(MealKitchenFeature.POST_PUBLICATIONS)
    ...
    @Test fun sameSourceAndCompositionDoNotMakeCrossPurposeSnapshotsInterchangeable() = runTest {
        val f = Fixture(StandardTestDispatcher(testScheduler))
        val publication = f.bind(MealKitchenFeature.POST_PUBLICATIONS)
        val publishAdmission = PublicationPrincipalAdmission(f.composition, publication, f.source)
        try {
            val draftPrincipal = f.run { f.admission.resolve(it) }
            val publicationPrincipal = f.composition.operate(publication) {
                publishAdmission.resolve(f.composition.composerPermit(publication))
            }
            assertEquals(draftPrincipal.canonicalUserId, publicationPrincipal.canonicalUserId)
            assertNotSame(draftPrincipal.binding, publicationPrincipal.binding)
            val requirements = f.source.requirements
            f.run { permit -> failure(FailureReason.STALE_SESSION) { f.admission.requireCurrent(permit, publicationPrincipal) } }
            f.composition.operate(publication) {
                failure(FailureReason.STALE_SESSION) {
                    publishAdmission.requireCurrent(f.composition.composerPermit(publication), draftPrincipal)
                }
            }
            assertEquals(requirements, f.source.requirements)
            f.run { f.admission.requireCurrent(it, draftPrincipal) }
            f.composition.operate(publication) {
                publishAdmission.requireCurrent(f.composition.composerPermit(publication), publicationPrincipal)
            }
            assertEquals(requirements + 2, f.source.requirements); f.assertNoIo()
        } finally { f.close() }
    }

    @Test fun synchronousDataGuardsRemainPurposeBoundWithoutRestoringOperationAuthority() = runTest {
        val f = Fixture(StandardTestDispatcher(testScheduler))
        val publication = f.bind(MealKitchenFeature.POST_PUBLICATIONS)
        val publishAdmission = PublicationPrincipalAdmission(f.composition, publication, f.source)
        try {
            ...
            withContext(f.dispatcher) {
                f.admission.requireCurrentNow(draftPrincipal)
                publishAdmission.requireCurrentNow(publicationPrincipal)
                failure(FailureReason.STALE_SESSION) { f.admission.requireCurrentNow(publicationPrincipal) }
                failure(FailureReason.STALE_SESSION) { publishAdmission.requireCurrentNow(draftPrincipal) }
                f.source.generation = Any()
                failure(FailureReason.STALE_SESSION) { f.admission.requireCurrentNow(draftPrincipal) }
                failure(FailureReason.STALE_SESSION) { publishAdmission.requireCurrentNow(publicationPrincipal) }
            }
            assertEquals(2, f.source.resolves); assertEquals(2, f.source.requirements); f.assertNoIo()
        } finally { f.close() }
    }

    @Test fun retainedSessionComparisonUsesPrivateReferencesAndDoesNotClaimCurrentness() = runTest {
        val f = Fixture(StandardTestDispatcher(testScheduler)); val foreign = SessionBoundary()
        try {
            val actual = PublicationSessionBinding(f.access, f.boundary)
            val anotherBinding = PublicationSessionBinding(f.access, f.boundary)
            assertTrue(actual.sameRetainedSession(actual)); assertTrue(actual.sameRetainedSession(anotherBinding))
            assertTrue(anotherBinding.sameRetainedSession(actual))
            for (substitute in listOf(
                PublicationSessionBinding(access(f.lease, ORIGIN, f.store), f.boundary),
                PublicationSessionBinding(access(f.lease, ORIGIN, Store()), f.boundary),
                PublicationSessionBinding(access(f.lease, OTHER_ORIGIN, f.store), f.boundary),
                PublicationSessionBinding(f.access, foreign),
                PublicationSessionBinding(access(foreign.activate(f.scope), ORIGIN, f.store), foreign),
                PublicationSessionBinding(access(foreign.activate(StorageScope("elsewhere", ActorKind.ACCOUNT, "opaque-owner")), ORIGIN, f.store), foreign))) {
                assertFalse(actual.sameRetainedSession(substitute)); assertFalse(substitute.sameRetainedSession(actual))
            }
            f.boundary.clear()
            assertFalse(f.boundary.isCurrent(f.lease))
            assertTrue(actual.sameRetainedSession(anotherBinding)) // Immutable equality is NOT live authority.
            assertEquals(0, f.source.resolves); assertEquals(0, f.source.syncChecks); f.assertNoIo()
        } finally { foreign.clear(); f.close() }
    }
    ...
    private class Fixture(val dispatcher: CoroutineDispatcher,
        ...
        val borrower = bind(MealKitchenFeature.POST_DRAFTS)
        val source = SyntheticSource(access, boundary)
        val admission = ReviewedDraftPrincipalAdmission(composition, borrower, source)
