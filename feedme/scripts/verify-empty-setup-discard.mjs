...
// Confirmed readable-empty setup discard only; no generalized orphan or planned-write repair.
// Requires existing JDK17, SDK36, local PostgreSQL binaries and a booted local emulator.
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const destination = path.join(root, 'docs/verification/empty-setup-discard');
...
const report = {
  ...
  scope: 'Bounded, explicitly confirmed discard of an incomplete setup with readable selected metadata and empty private state: exact credential incarnation/work origin/data target, typed durable setup-discard intent and empty-only native work/private-data cleanup. Every private record counts, including tombstones; an earlier empty observation never permits deleting later private data. Already-open resource inspection and exact recovery do not enumerate owners, silently initialize missing factories, merge guests or infer identity. Tests use fake or synthetic identity verification and native confirmation orchestration, not an actual identity provider or user-facing confirmation UI. Fresh shared/server/isolated-PostgreSQL/Node regressions, five Android library builds/lint, and isolated real native storage, credential, cancellation and separately invoked runtime/diagnostic/discard integration tests. Only storage has two separate instrumentation-process stages; no process-hard-kill or power-loss proof. Credential factory support remains gated to API 27+ (API 26 NOT_CONFIGURED). Hardlink platform-denial and notification branches are retained explicitly; platform denial is not existing-hardlink guard proof, and permission-unavailable is not delivered-notification cancellation proof. Native-factory failures, unreadable control/work/data evidence, unknown file orphans beyond exact authenticated credential CREATE plans, refresh/private-data/work planned-write recovery, full first-factory initialization recovery, user-facing deletion flows, production timer receiver/worker and app scheduling/UI/recovery wiring, actual provider sign-in, physical-device, iOS and release gates remain outside this receipt.',
