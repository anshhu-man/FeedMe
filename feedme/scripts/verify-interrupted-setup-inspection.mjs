...
// Source-bound already-open interrupted setup inspection; no confirmation or recovery effects.
...
const destination = path.join(root, 'docs/verification/interrupted-setup-inspection');
...
const report = {
  ...
  scope: 'M1.05d.5b.2b.3.6a already-open interrupted composite setup inspection only: authenticate and bracket the exact independent PendingSetup and its owner/configuration with credential-plan metadata fingerprints, private-data activation metadata plus separate sole-binding validation, and work-origin revision evidence. Credential observation authenticates scope and the issuing install without reading token or credential-blob contents; SELECTED does not establish usable credentials. Comparison fingerprints and repeated matching observations are not write or durability acknowledgements and authorize no selection, completion, cleanup, confirmation or abort. Closed/poisoned credential managers remain blocked; diagnostics do not open factories, initialize missing stores, bypass native partial inventory, or extract a nested credential plan into legacy abort. All earlier actual runtime late-publication, changed binding/work/control acknowledgement, exact native cleanup, metadata, parser and component regressions remain included. The retained 27 identity-bound VFS labels and five separate-process stages remain distinct from application response-loss wrappers: injected SQLite VFS errors are not OS errno failures or physical power loss, and direct owned-connection tests do not prove public hot-journal recovery. Native runtime fixtures use an explicitly synthetic verifier, not actual provider authentication or bootstrap. Confirmed composite abort, startup/native-factory recovery, integrated process-separation failures, typed provider/configuration approval, first-factory/close recovery, API26 native credential/recovery, product UI/domain wiring, physical devices, full Xcode/iOS and release acceptance remain open. Fresh Kotlin/server/isolated PostgreSQL/Node tests, five Android library builds/lint and test-only four-ABI JNI provenance remain source-bound. The historical demo and public GitHub snapshot are unchanged; no external deployment or publication.',
...
try {
  ...
  if (!/^# tests 122$/m.test(nodeLog) || !/^# pass 122$/m.test(nodeLog) || !/^# fail 0$/m.test(nodeLog) || !/^# skipped 0$/m.test(nodeLog))
    throw new Error('Expected all 122 Node regressions without skips.');
  for (const [directory, name, expected] of [
    ...
    ['shared/storage/build/test-results/jvmTest', 'storage', 424],
    ['shared/sync/build/test-results/jvmTest', 'sync', 103],
    ['shared/kitchen/build/test-results/jvmTest', 'kitchen', 142],
    ['shared/session/build/test-results/jvmTest', 'session', 462],
  ...
  report.node = {tests: 122, failures: 0, errors: 0, skipped: 0};
  report.androidLint = [];
