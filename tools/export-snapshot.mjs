...
function copy(sourceRelative, publishedRelative) {
  ...
  let published = original, redactions = 0, unrelatedInstrumentationRedactions = 0;
  if (!original.includes(0)) {
    ...
    const minimized = sanitizeDiagnostics(sourceRelative, safe);
    unrelatedInstrumentationRedactions = minimized.unrelatedInstrumentationRedactions;
  ...
  manifest.files.push({source: sourceRelative, path: publishedRelative, originalBytes: original.length, originalSha256: sha(original), bytes: published.length, sha256: sha(published), homePathRedactions: redactions, unrelatedInstrumentationRedactions});
}
copy('feedme', 'feedme');
...
console.log(JSON.stringify({copied: manifest.files.length, bytes: manifest.files.reduce((sum, item) => sum + item.bytes, 0), redactedFiles: manifest.files.filter(item => item.homePathRedactions || item.unrelatedInstrumentationRedactions).length, homePathRedactions: manifest.files.reduce((sum, item) => sum + item.homePathRedactions, 0), unrelatedInstrumentationRedactions: manifest.files.reduce((sum, item) => sum + item.unrelatedInstrumentationRedactions, 0), excluded: manifest.excluded.length}));
