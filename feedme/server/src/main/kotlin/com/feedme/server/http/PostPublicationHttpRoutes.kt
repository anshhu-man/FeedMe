...
internal class PostPublicationHttpInput private constructor(
    ...
    suspend fun readBody(channel: ByteReadChannel, validator: ContractBodyValidator): JsonObject {
        val buffer = ByteArray(PostPublicationHttpConfiguration.MAX_REQUEST_BYTES + 1)
        var size = 0
        try {
            while (true) {
                currentCoroutineContext().ensureActive()
                val count = channel.readAvailable(buffer, size, buffer.size - size)
                if (count == -1) { channel.closedCause?.let { throw it }; break }
                size += count
                if (size == buffer.size) invalidPublication()
            }
            currentCoroutineContext().ensureActive()
            if (contentLength != null && contentLength != size.toLong()) invalidPublication()
            return body(buffer.copyOf(size), validator)
        } catch (cancelled: CancellationException) { throw cancelled }
        catch (_: IOException) { currentCoroutineContext().ensureActive(); invalidPublication() }
