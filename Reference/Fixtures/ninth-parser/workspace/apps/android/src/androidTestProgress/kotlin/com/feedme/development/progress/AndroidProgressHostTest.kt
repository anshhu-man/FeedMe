...
class AndroidProgressHostTest {
    ...
    @Test fun bActualLauncherRequiresConsentAndRetainsCookingAcrossActivityRecreation() {
        ...
        try {
            click("Inspect retained state"); waitText("Start synthetic preview"); screenshot("progress-start")
            ...
            // Exercise the actual restored-offset hazard deterministically: the review action
            // must remain reachable from below, without preparing or dispatching another start.
            scrollToEnd()
            await { textNodes("Review prepared start").none { it.isVisibleToUser } }
            assertFalse(textNodes("Review prepared start").any { it.isVisibleToUser })
            ...
            waitText("SYNTHETIC ACCOUNT / CONTENT / SERVICE")
    ...
    @Test fun cAcknowledgedCloseAndResumeKeepExactLocalDraftCookingAndServiceLineage() {
        ...
        try {
            main {
                startOwner(); val experience = checkNotNull(owner.experience.value); value(experience.restore()); meal(experience)
                value(experience.prepareCooking()); value(experience.confirmCooking(checkNotNull(experience.cookingNavigation.value.confirmation)))
                val plan = checkNotNull(experience.cooking.states.value.plan).id.value
                value(owner.close()); assertNull(owner.experience.value); assertNull(experience.forms.value.values)
                value(owner.inspectStartup()); assertEquals(ProgressHostPhase.RESUME_AVAILABLE, owner.states.value.phase)
                value(owner.resume()); val restored = checkNotNull(owner.experience.value); assertNotSame(experience, restored)
                value(restored.restore())
                ...
                value(restored.cooking.refresh()); assertEquals(plan, restored.cooking.states.value.plan!!.id.value)
                assertFalse(restored.cooking.states.value.serverAcknowledged) // A GET is not a new mutation acknowledgement.
            }
        } finally { main { resetOwner() } }
    }
