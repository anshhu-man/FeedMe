...
// Lease-free, install-bound work-origin plans with exact native selection/replay evidence.
...
const destination = path.join(root, 'docs/verification/work-origin-planning');
...
const report = {
  ...
  scope: 'M1.05d.5b.2b.3.2 bounded component only: lease-free read-only planning captures one UUID and the exact idle predecessor; a purpose-fixed proof binds it to the existing native work owner, key and ledger. Selection and every selected replay require fresh changed-CAS acknowledgement and exact readback. The setup-selected marker cannot be ordinary resumed, scheduled, executed or retired. No provider verification, active lease, binding or OS work is granted. Current exact-process and actual-engine VFS evidence is retained below; injected SQLite VFS errors are not OS errno failures or physical power loss, and direct owned-connection fixtures do not prove public hot-journal recovery. Composite pending intent, setup sealing/explicit abort, cross-store binding and publication remain separate tasks. Provider/bootstrap/UI, first-factory repair, hard-kill/power-loss, physical devices, API26 recovery, iOS and release acceptance remain open. Fresh shared/server/isolated PostgreSQL/Node regressions, five Android library builds/lint, and test-only four-ABI JNI provenance are included. The historical demo APK and public GitHub snapshot are unchanged; no new deployment or publication.',
...
try {
  ...
  for (const [directory, name, expected] of [
    ...
    ['shared/storage/build/test-results/jvmTest', 'storage', 339],
    ['shared/sync/build/test-results/jvmTest', 'sync', 103],
    ['shared/kitchen/build/test-results/jvmTest', 'kitchen', 142],
    ['shared/session/build/test-results/jvmTest', 'session', 290],
