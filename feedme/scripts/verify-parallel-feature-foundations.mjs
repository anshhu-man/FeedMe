...
// Source-bound parallel feature foundations; not complete V1 features or release acceptance.
...
const parallelCounts = {planning: 40, server: 55, postgresql: 73};
...
const destination = path.join(root, 'docs/verification/parallel-feature-foundations');
...
const sourceFiles = () => [
  ...
  ...['core', 'contracts', 'transport', 'storage', 'sync', 'kitchen', 'session', 'planning'].flatMap(module => [
...
const report = {
  ...
  scope: "Parallel V1 component foundations, not whole-feature or release completion: retained existing-only CONTROL ownership captures the exact fixed owner and sole bounded encrypted ledger before exposing trusted read/CAS, preserving partial opening, cancellation, changed-CAS acknowledgements and truthful close. It does not parse pending/complete session semantics, authenticate nested plans, initialize missing state, resume ordinary stores, run GC or grant access. Existing credential/data/work owners and borrowed-store composite-abort regressions remain; all-owning startup and close-before-Complete composition remain separate gates. The planning module checks deterministic reviewed-input cooking decisions; it is not approved catalog/provider or native UI integration. PostgreSQL social persistence and explicit authorization are component acceptance, not deployed circles, provider authentication or full social journeys. Final package-specific counts are frozen before execution. Approved V1 remains44 P1 features with10 deferred; its manifest gate enforces planning/build partition only and preserves shared resources, not runtime feature gating or release certification. All39 existing VFS labels, five controlled process stages and four-ABI test-helper isolation remain; new lifetime hook failures are not new OS errno or engine injection. Provider/UI, full startup recovery, hot-journal recovery, hard-kill/power-loss/physical-device evidence, API26 recovery, full Xcode/iOS, signed distribution and release acceptance remain open. No deployment, store release, public upload or provider action is performed.",
...
function verifyInjectionPackaging() {
  ...
  const forbidden = [
    ...['storage', 'transport', 'sync', 'kitchen', 'session', 'planning'].map(module => `shared/${module}/build/outputs/aar/${module}-debug.aar`),
...
try {
  ...
  await run('gradle', './gradlew', [
    ':shared:core:jvmTest', ':shared:contracts:jvmTest', ':shared:transport:jvmTest', ':shared:storage:jvmTest', ':shared:sync:jvmTest', ':shared:kitchen:jvmTest', ':shared:session:jvmTest', ':shared:planning:jvmTest',
  ...
  for (const [directory, name, expected] of [
    ...
    ['shared/session/build/test-results/jvmTest', 'session', 557],
  ...
  report.androidLint = [];
  for (const module of ['storage', 'transport', 'sync', 'kitchen', 'session', 'planning']) {
