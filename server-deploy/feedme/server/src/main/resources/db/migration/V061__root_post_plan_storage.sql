-- Exact account Post -> Make Mine storage format 6. No new tables, privileges,
-- copied-source permission or provider effects. Historical format 1--5 bodies stay intact.
ALTER TABLE planning.plan_requests
    DROP CONSTRAINT plan_requests_storage_format_check,
    ADD CONSTRAINT plan_requests_storage_format_check CHECK (storage_format IN (1,2,3,4,5,6)),
...
ALTER TABLE planning.plan_requests ADD CONSTRAINT planning_request_representation CHECK (
    ...
    OR (storage_format IN (3,4,5,6) AND evidence_text IS NOT NULL AND evidence_hash IS NOT NULL AND ordered_ids IS NULL
        AND manifest_id IS NULL AND create_command_key IS NULL AND command_request_sha256 IS NULL)
);
...
ALTER TABLE planning.plan_requests ADD CONSTRAINT planning_derived_request_fields CHECK (
    ...
    OR (storage_format IN (4,5,6) AND actor_kind='account' AND derived_operation='createPlan' AND derived_operation IS NOT NULL
...
CREATE UNIQUE INDEX root_post_original_command ON planning.plan_requests(
    environment,actor_kind,principal_id,derived_operation,derived_command_key) WHERE storage_format=6;

ALTER TABLE planning.plans
    DROP CONSTRAINT plans_storage_format_check,
    ADD CONSTRAINT plans_storage_format_check CHECK (storage_format IN (1,2,3,4,5,6));
...
ALTER TABLE planning.plans ADD CONSTRAINT plans_position_check CHECK (
    ...
    OR (storage_format IN (3,4,5,6) AND position=(CASE WHEN status='ready' THEN 0 ELSE -1 END))
);
ALTER TABLE planning.plans ADD CONSTRAINT planning_root_post_fields CHECK (
    storage_format<>6 OR (actor_kind='account' AND parent_plan_id IS NULL AND next_cursor_hash IS NULL)
);
CREATE UNIQUE INDEX root_post_one_result ON planning.plans(environment,actor_kind,principal_id,request_id)
    WHERE storage_format=6;

CREATE FUNCTION planning.reject_root_post_request_update() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'Root Post Plan original is immutable' USING ERRCODE='23514'; END;
$$;
CREATE TRIGGER root_post_request_immutable BEFORE UPDATE ON planning.plan_requests
    FOR EACH ROW WHEN (OLD.storage_format=6) EXECUTE FUNCTION planning.reject_root_post_request_update();

CREATE FUNCTION planning.guard_root_post_request_insert() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE document jsonb; request jsonb; source jsonb; post_source jsonb;
...
BEGIN
    ...
    post_source := document->'provenance'->'postSource';
    IF NEW.request_hash IS DISTINCT FROM encode(sha256(convert_to(NEW.request_text,'UTF8')),'hex')
        ...
        OR document->'provenance'->>'kind' IS DISTINCT FROM 'postRootAdaptation'
        ...
        OR jsonb_typeof(request->'sourcePostId') IS DISTINCT FROM 'string'
        OR NOT coalesce(request->>'sourcePostId' ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$',false)
        OR request ?| ARRAY['sourceRecipeVersionId','savedRecipeId']
        OR jsonb_typeof(request->'sourcePostVersion') IS DISTINCT FROM 'number'
        OR NOT coalesce(request->>'sourcePostVersion' ~ '^[1-9][0-9]*$',false)
        OR jsonb_typeof(post_source) IS DISTINCT FROM 'object'
        OR NOT coalesce(post_source ?& ARRAY['environment','ownerId','postId','postVersion','attachment','recipeText','recipeSha256'],false)
        OR (post_source-ARRAY['environment','ownerId','postId','postVersion','attachment','recipeText','recipeSha256']) IS DISTINCT FROM '{}'::jsonb
        OR post_source->>'environment' IS DISTINCT FROM NEW.environment
        OR post_source->'postId' IS DISTINCT FROM request->'sourcePostId'
        OR post_source->'postVersion' IS DISTINCT FROM request->'sourcePostVersion'
        OR NOT coalesce(post_source->>'ownerId' ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$',false)
        OR jsonb_typeof(post_source->'attachment') IS DISTINCT FROM 'object'
        OR post_source->'attachment'->>'rightsBasis' IS DISTINCT FROM 'catalogRedistributable'
        OR post_source->'attachment'->>'reviewStatus' IS DISTINCT FROM 'reviewed'
        OR jsonb_typeof(post_source->'recipeText') IS DISTINCT FROM 'string'
        OR (post_source->>'recipeText')::jsonb IS DISTINCT FROM (source->>'recipeText')::jsonb
        OR (post_source->>'recipeText')::jsonb->'id' IS DISTINCT FROM source->'recipeVersionId'
        OR NOT coalesce(post_source->>'recipeSha256' ~ '^[0-9a-f]{64}$',false)
        ...
        RAISE EXCEPTION 'Root Post Plan request differs from original' USING ERRCODE='23514';
    END IF;
    RETURN NEW;
END;
$$;
CREATE TRIGGER root_post_request_original BEFORE INSERT ON planning.plan_requests
    FOR EACH ROW WHEN (NEW.storage_format=6) EXECUTE FUNCTION planning.guard_root_post_request_insert();

CREATE FUNCTION planning.guard_root_post_plan_insert() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
    ...
    IF NOT FOUND OR lineage.storage_format<>6 OR lineage.actor_kind<>'account' OR lineage.version<>1
        ...
        RAISE EXCEPTION 'Root Post Plan request unavailable' USING ERRCODE='23514';
    END IF;
    ...
    expected_proof := jsonb_build_object(
        'version',6,'kind','rootPostPlan','owner',context->'owner','operationId',lineage.derived_operation,
    ...
    IF NEW.snapshot_hash IS DISTINCT FROM encode(sha256(convert_to(NEW.snapshot_text,'UTF8')),'hex')
        ...
        OR body ? 'parentPlanId'
        OR body->'sourcePostId' IS DISTINCT FROM context->'provenance'->'postSource'->'postId'
        ...
        RAISE EXCEPTION 'Root Post Plan bytes differ from row' USING ERRCODE='23514';
    END IF;
    RETURN NEW;
END;
$$;
CREATE TRIGGER root_post_plan_original BEFORE INSERT ON planning.plans
    FOR EACH ROW WHEN (NEW.storage_format=6) EXECUTE FUNCTION planning.guard_root_post_plan_insert();

CREATE FUNCTION planning.require_root_post_plan_complete() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
    ...
    IF NOT FOUND OR lineage.storage_format<>6 OR lineage.actor_kind<>'account' OR lineage.version<>1
        ...
        RAISE EXCEPTION 'Root Post Plan request incomplete at commit' USING ERRCODE='23514';
    END IF;
    ...
    IF NOT FOUND OR result.storage_format<>6 OR result.parent_plan_id IS NOT NULL OR result.version<>1
        OR result.created_at IS DISTINCT FROM lineage.created_at OR result.next_cursor_hash IS NOT NULL
        OR result.proof_text::jsonb->'version' IS DISTINCT FROM '6'::jsonb
        OR NOT isfinite(lineage.expires_at) OR lineage.expires_at<=at_time THEN
        RAISE EXCEPTION 'Root Post Plan result incomplete at commit' USING ERRCODE='23514';
    END IF;
    RETURN NULL;
END;
$$;
CREATE CONSTRAINT TRIGGER root_post_request_requires_result AFTER INSERT ON planning.plan_requests
    DEFERRABLE INITIALLY DEFERRED FOR EACH ROW WHEN (NEW.storage_format=6)
    EXECUTE FUNCTION planning.require_root_post_plan_complete();
CREATE CONSTRAINT TRIGGER root_post_plan_requires_request AFTER INSERT ON planning.plans
    DEFERRABLE INITIALLY DEFERRED FOR EACH ROW WHEN (NEW.storage_format=6)
    EXECUTE FUNCTION planning.require_root_post_plan_complete();
...
REVOKE ALL ON FUNCTION planning.reject_root_post_request_update(), planning.guard_root_post_request_insert(),
    planning.guard_root_post_plan_insert(), planning.require_root_post_plan_complete() FROM PUBLIC;
