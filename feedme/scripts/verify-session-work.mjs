...
// Isolated durable native-work admission/cancellation, not actual app scheduling or a release.
// Requires existing JDK17, SDK36, local PostgreSQL binaries and a booted local emulator.
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const destination = path.join(root, 'docs/verification/session-work');
...
const report = {
  ...
  scope: 'Durable native-work registry, strict effect-index codec and separate encrypted install-local work storage; lease/origin/ticket-fenced admission, exact native cancellation and local-callback policy gates. Fresh shared/server/isolated-PostgreSQL/Node regressions, five Android library builds/lint, isolated native storage, credential, cancellation and separate real-store/runtime integration tests. Runtime identity verification remains fake or synthetic, not an actual provider. Only storage includes two separate instrumentation-process stages; credential/work/integration tests exercise controlled reopen and bounded native operations, not a process-hard-kill. Android credential factory remains gated to API 27+ (API 26 NOT_CONFIGURED); platform-denied hardlink fixtures do not prove existing-hardlink guard execution. Notification cancellation branches are recorded, and permission-unavailable is not delivered-notification proof. No production timer receiver/worker, app scheduling/recovery wiring, verified identity-provider sign-in/refresh, actual UI/logout/account-switch workflow, physical-device, power-loss, iOS or release claim.',
