...
const destination = path.join(root, 'docs/verification/kitchen-repositories');
...
const report = {
  ...
  scope: 'Private saved/cooking repositories with real encrypted-SQLite reopen and command integration; fresh shared/server/isolated-PostgreSQL/Node regressions plus Android library builds/lint. No actual auth/backend/UI workflow, native device execution, iOS, hard-kill crash or store release claim.',
...
try {
  await run('gradle', './gradlew', [
    ...
    ':shared:kitchen:assembleDebug', ':shared:kitchen:lintDebug',
