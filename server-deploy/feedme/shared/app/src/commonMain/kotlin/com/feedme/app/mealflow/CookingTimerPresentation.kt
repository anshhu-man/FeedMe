...
/** A rendered page binding, not authority to mutate or a native scheduling ticket. */
class CookingTimerRenderedAction internal constructor(
    internal val owner: CookingTimerFlowController,
    internal val state: CookingTimerFlowState,
    internal val navigation: CookingNavigation,
    internal val removal: CookingTimerRemoval?,
) {
    override fun toString() = "CookingTimerRenderedAction(<redacted>)"
}
...
class CookingTimerRemoval internal constructor(internal val sessionId: String, internal val planId: String,
    internal val revision: Long, internal val timerId: String, internal val action: CookingTimerRenderedAction? = null) {
...
internal class CookingTimerUiOwner(private val boundary: SessionBoundary, private val lease: SessionLease) {
    ...
    fun present(session: String, plan: String, revision: Long, timer: String, action: CookingTimerRenderedAction? = null) {
        if (!closed && boundary.isCurrent(lease)) mutable.value = CookingTimerRemoval(session, plan, revision, timer, action)
    }
...
class CookingTimerRow internal constructor(val id: String, val label: String, val timing: String,
    ...
    internal val stepId: String? = null, internal val durationSeconds: Long? = null,
    internal val remainingMillis: Long? = null, internal val timingKind: CookingTimerTiming? = null) {
...
class CookingTimerScreenState internal constructor(val visible: Boolean, val busy: Boolean, val eligible: Boolean,
    ...
    internal val sessionId: String? = null, internal val planId: String? = null,
    internal val localRevision: Long? = null, internal val selectedStepId: String? = null) {
    ...
    companion object {
        fun from(state: CookingTimerFlowState): CookingTimerScreenState {
            ...
            return CookingTimerScreenState(state.visible, state.busy, eligible,
                ...
                pin?.sessionId, pin?.planId, pin?.localRevision, state.selectedStepId)
        }
    }
}
...
internal fun timerRow(view: CookingTimerView, eligibleLabels: Map<String, String>, busy: Boolean): CookingTimerRow =
    CookingTimerRow(view.timerId, eligibleLabels[view.stepId] ?: "Pinned step label unavailable", timerTiming(view.timing?.timing),
        ...
        view.stepId, view.durationSeconds, view.timing?.remainingMillis, view.timing?.timing)
