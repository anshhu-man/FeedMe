...
class AndroidProgressTimerHostTest {
    ...
    private inner class Fixture {
        ...
        fun open() {
            ...
            click("Timers for this step"); waitText("A little less clock-watching."); settled()
    ...
    @Test fun aVisibleControlsKeepTwoExactTimersAndRequireExplicitRemovalConsent() = withHost { f ->
        ...
        // Pause is below its status pill. Observe from the top before forward-only search.
        top()
    ...
    @Test fun bBackAndActivityRecreationRetainExactTimerOwnerWithoutPauseOrNewCommand() = withHost { f ->
        ...
        click("Timers for this step"); waitText("A little less clock-watching."); f.settled()
    ...
    private fun setDuration(text: String) {
        top()
        // The second timer uses the actual optional setup disclosure. Selecting or editing
        // a duration still does not start a timer; Fixture.start dispatches Start separately.
        if (main { owner.experience.value?.timers?.states?.value?.snapshot?.timers?.isNotEmpty() == true })
            click("Add another timer")
        waitText("Timer duration", scroll = true)
