...
// Source-bound retained credential/data recovery owners; full startup composition remains open.
...
const destination = path.join(root, 'docs/verification/startup-recovery-owners');
...
const report = {
  ...
  scope: "M1.05d.5b.2b.3.6d.1 retained credential/private-data recovery owner foundations: synchronous owner construction before native I/O, exact existing-only scope/plan authentication and full observations, retained partial acquisition and failed initialization, close-only failed/cancelled opening, explicit staged close acknowledgements and retryable pre-stage release faults. Native ambiguous post-close simulation retains terminal process reservations and never retries a numeric descriptor. The actual scope/plan-pinned APIs expose no ordinary handle, token, lease, initialization, migration or GC. Known lower-store poisoning remains authoritative. Existing-only work recovery, all-owning startup factory composition, owned-close-before-Complete coordination, integrated process-separation, real-provider/UI, physical-device/power-loss, public hot-journal, API26/iOS and release acceptance remain open. All previous native tests,39 VFS labels/five controlled process stages and isolated four-ABI helper checks remain. Application hook/receipt-loss failures are not new SQLite VFS or OS power-loss faults. Demo and public repository snapshot are unchanged; no deployment or publication.",
...
try {
  ...
  for (const [directory, name, expected] of [
    ...
    ['shared/storage/build/test-results/jvmTest', 'storage', 488],
    ['shared/sync/build/test-results/jvmTest', 'sync', 103],
    ['shared/kitchen/build/test-results/jvmTest', 'kitchen', 142],
    ['shared/session/build/test-results/jvmTest', 'session', 529],
