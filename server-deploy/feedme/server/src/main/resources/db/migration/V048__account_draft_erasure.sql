-- LOCAL exact-owned unpublished/media-free draft cleanup only. No external storage,
-- publication erasure, new API grant, live rollout or full F49 completion is installed.
-- Legacy unattributed events remain unknown; no payload-to-owner backfill occurs.
-- V042 event ownership, V044 capability/checkpoint and V046 manifest guards are unchanged.

CREATE FUNCTION platform.guard_account_draft_owner_write() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,pg_temp SET row_security=off
AS $feedme_draft_owner$
DECLARE account_status text; principal_status text; private_id uuid;
BEGIN
 IF TG_RELID NOT IN ('platform.post_drafts'::regclass,'platform.post_draft_heads'::regclass,
     'platform.media_draft_lifecycles'::regclass)
   OR current_setting('session_replication_role')<>'origin'
   OR current_setting('transaction_isolation')<>'read committed' THEN
  RAISE EXCEPTION 'Account draft owner transaction is unavailable' USING ERRCODE='23514';
 END IF;
 IF TG_OP='UPDATE' AND ROW(NEW.environment,NEW.owner_user_id) IS DISTINCT FROM ROW(OLD.environment,OLD.owner_user_id) THEN
  RAISE EXCEPTION 'Account draft ownership is immutable' USING ERRCODE='23514';
 END IF;
 -- These historical roots have no account FK. Lock the actual user before the
 -- mapped private principal, in the same order as deletion acceptance.
 SELECT u.status INTO account_status FROM identity.users u
  WHERE u.environment=NEW.environment AND u.id=NEW.owner_user_id FOR SHARE NOWAIT;
 IF NOT FOUND OR account_status NOT IN ('active','suspended') THEN
  RAISE EXCEPTION 'Account draft owner is unavailable' USING ERRCODE='23514';
 END IF;
 SELECT p.id,p.status INTO private_id,principal_status FROM identity.principals p
  WHERE p.environment=NEW.environment AND p.user_id=NEW.owner_user_id AND p.kind='user'
    AND p.guest_session_id IS NULL FOR SHARE NOWAIT;
 IF NOT FOUND OR private_id IS NULL OR principal_status NOT IN ('active','suspended') THEN
  RAISE EXCEPTION 'Account draft principal is unavailable' USING ERRCODE='23514';
 END IF;
 IF EXISTS(SELECT 1 FROM identity.account_deletion_jobs j WHERE j.environment=NEW.environment
   AND (j.user_id=NEW.owner_user_id OR j.principal_id=private_id)) THEN
  RAISE EXCEPTION 'Accepted deletion fences account drafts' USING ERRCODE='23514';
 END IF;
 -- This is a lifecycle fence, not sign-in, current Terms or publication authority.
 RETURN NEW;
END;
$feedme_draft_owner$;
REVOKE ALL ON FUNCTION platform.guard_account_draft_owner_write() FROM PUBLIC;
CREATE TRIGGER account_draft_owner_fence BEFORE INSERT OR UPDATE ON platform.post_drafts
 FOR EACH ROW EXECUTE FUNCTION platform.guard_account_draft_owner_write();
CREATE TRIGGER account_draft_owner_fence BEFORE INSERT OR UPDATE ON platform.post_draft_heads
 FOR EACH ROW EXECUTE FUNCTION platform.guard_account_draft_owner_write();
CREATE TRIGGER account_draft_owner_fence BEFORE INSERT OR UPDATE ON platform.media_draft_lifecycles
 FOR EACH ROW EXECUTE FUNCTION platform.guard_account_draft_owner_write();

-- Separate family admission preserves V042's exact historical function body.
CREATE FUNCTION platform.protect_post_draft_event_owner() RETURNS trigger
LANGUAGE plpgsql SECURITY INVOKER SET search_path=pg_catalog,pg_temp
AS $feedme_draft_event_owner$
BEGIN
 IF NEW.event_type='social.post_draft.changed.v1' THEN
  IF NEW.owner_environment IS NULL OR NEW.owner_kind IS DISTINCT FROM 'account' OR NEW.owner_id IS NULL
    OR jsonb_typeof(NEW.payload->'environment') IS DISTINCT FROM 'string'
    OR jsonb_typeof(NEW.payload->'ownerId') IS DISTINCT FROM 'string'
    OR NEW.payload->>'environment' IS DISTINCT FROM NEW.owner_environment
    OR NEW.payload->>'ownerId' IS DISTINCT FROM NEW.owner_id::text THEN
   RAISE EXCEPTION 'Post draft event requires exact account ownership' USING ERRCODE='23514';
  END IF;
 END IF;
 RETURN NEW;
END;
$feedme_draft_event_owner$;
REVOKE ALL ON FUNCTION platform.protect_post_draft_event_owner() FROM PUBLIC;
CREATE TRIGGER outbox_post_draft_owner_required BEFORE INSERT ON platform.outbox
 FOR EACH ROW EXECUTE FUNCTION platform.protect_post_draft_event_owner();
...
DECLARE
 ...
 account_ops constant text[]:=ARRAY['bootstrapAccount','logoutSession','acceptAccountTerms','updateMe','requestAccountDeletion',
   'createPostDraft','updatePostDraft','deletePostDraft'];
