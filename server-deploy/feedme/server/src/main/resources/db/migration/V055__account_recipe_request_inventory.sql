-- Local inventory extension only: no shared-history erasure, retention policy, grants or live activation.
-- V053 and V054 bytes/guards remain unchanged. The only replacement is the fixed
-- core purge with one exact new relation fence and account-specific related-data holds.
-- Existing unknown-relation, unattributed-event, lease, rollback and checkpoint rules remain.
