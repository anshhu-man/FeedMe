 ...
 */
internal class NativeTimerTestSession(private val context: Context, private val cancellation: NativeWorkCancellationPort, private val policy: NativeWorkExecutionPolicy) {
    ...
    @Volatile private var stage = NativeTimerFixtureStage.NEW

    private val mutex = Mutex()
    private var sandbox: NativeTimerTestSandbox? = null
    ...
    suspend fun retireAndClose(): PortResult<Unit> = withContext(NonCancellable + dispatcher) {
        serialized {
            ...
            closeOwners()
            stage = NativeTimerFixtureStage.REMOVE_RETIRED_FIXTURE
            withContext(Dispatchers.IO) { checkNotNull(sandbox).removeAfterRetirement() }
            removed = true
            stage = NativeTimerFixtureStage.REMOVED
        }
    }
    ...
    private fun reserveRoot() {
        stage = NativeTimerFixtureStage.RESERVE
    ...
    private suspend fun openRuntime() {
        stage = NativeTimerFixtureStage.RUNTIME_OPEN
        acquire({
            PrivateSessionRuntime.openReserved(checkNotNull(reservation), checkNotNull(control),
                ...
                cancellation, NativeWorkIdSource { UUID.randomUUID().toString() }, policy)
        }) { runtimeOwner = it }
    }
    ...
    private suspend fun closeOwners() {
        ...
        runtimeOwner?.let { stage = NativeTimerFixtureStage.RUNTIME_CLOSE; value(it.close()); runtimeOwner = null }
        work?.let { stage = NativeTimerFixtureStage.WORK_CLOSE; value(it.close()); work = null }
        data?.let { stage = NativeTimerFixtureStage.DATA_CLOSE; value(it.close()); data = null }
        credentials?.let { stage = NativeTimerFixtureStage.CREDENTIAL_CLOSE; value(it.close()); credentials = null }
        control?.let { stage = NativeTimerFixtureStage.CONTROL_CLOSE; value(it.close()); control = null }
    ...
    private suspend fun serialized(action: suspend () -> Unit): PortResult<Unit> = withContext(dispatcher) {
        mutex.withLock {
            ...
            catch (failure: NativeTimerFixtureFailure) {
                Log.e("FeedMeNativeTimerFixture", "stage=${stage.name};reason=${failure.reason.name}")
                PortResult.Failure(failure.reason)
            }
            catch (_: Exception) {
                Log.e("FeedMeNativeTimerFixture", "stage=${stage.name};reason=STORAGE_FAILURE")
                PortResult.Failure(FailureReason.STORAGE_FAILURE)
            }
        }
    }

    override fun toString() = "NativeTimerTestSession(<synthetic-test-only>)"
    ...
    companion object {
        ...
        private fun fail(reason: FailureReason): Nothing = throw NativeTimerFixtureFailure(reason)
...
private class NativeTimerFixtureFailure(val reason: FailureReason) : Exception("Native meal test fixture unavailable")

internal enum class NativeTimerFixtureStage {
