...
// Fifth source-bound kitchen HTTP/cooking-orchestration component batch; not complete V1 or release acceptance.
...
const parallelCounts = {sync: 108, kitchen: 171, planning: 40, mealflow: 170, app: 57, session: 593, server: 141, postgresql: 193, startupNative: 12};
...
const destination = path.join(root, 'docs/verification/parallel-kitchen-cooking');
...
const report = {
  ...
  scope: "Fifth parallel V1 pantry/preferences HTTP and shared cooking-orchestration component batch. Six explicitly configured kitchen HTTP operations use actual PostgreSQL KitchenStore, canonical exact JSON/decimal/version controls, original idempotency keys, monotonic delete/recreate incarnations and atomic receipt/outbox writes. Trusted provisioning is never a GET or public route. Mandatory verified account/guest, current lifecycle, reviewed references and controlled ingredient search adapters have no accepting production defaults. Real Ktor/CIO and PostgreSQL tests use explicitly synthetic identity/catalog authority; the cross-module planning fixture reads actual preference/pantry rows under the same exclusive principal lock, not a production provider/catalog implementation. Existing planning/social configured routes remain opt-in; default Main stays degraded/product503. The cooking controller uses the actual private kitchen queue and pinned acknowledged meal-flow selection, explicit start/retry consent, owned session/plan download, exact retained completion/progress and original command identity. Common tests use synthetic transports and stores; cooking server routes, native cooking UI/timers/save/share and live identity/catalog are not connected by this checkpoint. Exact private-write acknowledgement/readback, cancellation, lease-scoped learned recall and read-only origin/recall observations do not grant rights or turn lost effect receipts into successes. Native regression still comprises345 passed tests plus seven separately witnessed interrupted starts, with six synthetic presentation and nine actual retained-host tests using native encrypted storage and synthetic identity/transport. Existing39 injected-VFS labels/five controlled stages and four-ABI test-only helper packaging remain; runtime is API35 ARM64 only. This is not physical power-loss, OS-fsync failure, general hot-journal recovery, provider login, delayed native Save acknowledgement, full client/server cooking journey, API26/iOS, physical-device, signing or release acceptance. Notification/hardlink coverage depends on actual platform evidence. V1 remains44 P1 features with10 deferred; historical demo bytes remain unchanged.",
...
function verifyGlobalNativeInventory() {
  ...
  if (passed.length!==345 || interrupted.length!==7 || declared.length!==352 ||
      new Set([...passed,...interrupted]).size!==352 ||
  ...
  return {declared:352,passed:345,witnessedInterruptions:7,passedIdentities:passed.sort(),interruptedIdentities:[...interrupted].sort()};
}
...
try {
  ...
  if (report.nativeTests.tests !== 345) throw new Error('Expected 345 passed native tests; interrupted starts must never be counted.');
