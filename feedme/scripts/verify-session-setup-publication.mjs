...
// Source-bound private setup completion and late session publication; no external deployment.
...
const destination = path.join(root, 'docs/verification/session-setup-publication');
...
const report = {
  ...
  scope: 'M1.05d.5b.2b.3.5 private setup completion and late runtime publication: the actual runtime replacement keeps verified live credentials and exact native plans behind independent PendingSetup, obtains fresh changed private-binding acknowledgement plus exact readback, seals the exact setup work origin, acknowledges matching control completion and only then publishes a session lease and guarded access. Exact target/binding resume performs no garbage collection and does not itself authenticate a provider or control completion. Binding retries preserve schema/body but always increment and reseal the reserved record; three actual bundled-engine VFS tests exercise six initial/replay journal/database/post-unlink failures without returning a successful receipt. These new engine failures differ from application acknowledgement-loss wrappers used by coordinator/runtime tests. All prior 21 VFS labels, five distinct-process stages, component/native integration and strict source-bound metadata regressions are retained. Injected SQLite VFS errors are not OS errno failures or physical power loss; direct owned-connection fixtures do not prove public recovery-factory journal support. Native integration uses explicitly synthetic test verification, not real provider sign-in or bootstrap. Native credential poisoning/partial inventory, discarded in-memory retry authority after restart, first-factory recovery, composite startup/confirmed abort, approved typed provider configuration, UI/product wiring, API26 recovery, physical devices, full Xcode/iOS and release gates remain open. Five Android library lint/build checks, exact current artifacts and test-only four-ABI JNI provenance are included. Publication means a local process session lease, not GitHub/deployment: the historical demo and public repository snapshot are unchanged.',
...
try {
  ...
  for (const [directory, name, expected] of [
    ...
    ['shared/storage/build/test-results/jvmTest', 'storage', 396],
    ['shared/sync/build/test-results/jvmTest', 'sync', 103],
    ['shared/kitchen/build/test-results/jvmTest', 'kitchen', 142],
    ['shared/session/build/test-results/jvmTest', 'session', 418],
