...
class PlansStore(val environment: String, private val transactions: PgTransactions,
    ...
    fun getPlan(principal: VerifiedPlanningPrincipal, planId: UUID): StoredReply = read(principal) { c ->
    ...
    fun getPlanExplanation(principal: VerifiedPlanningPrincipal, planId: UUID, cursor: String? = null, limit: Int = 20): StoredReply = read(principal) { c ->
    ...
    private fun command(principal: VerifiedPlanningPrincipal, operation: String, key: UUID, path: Map<String, String>, body: WireDocument,
        ...
        val result = commands.execute(command, { authority.lockPrincipal(it, principal) }, {}, { c, cached ->
            val id = UUID.fromString(cached.body!!.jsonObject.text("id")); val plan = plan(c, principal, id); val lineage = lineage(c, principal, plan.requestId)
            authorizeStored(c, principal, lineage, plan, selection = plan.recipeId != null)
    ...
    private fun <T> read(principal: VerifiedPlanningPrincipal, action: (Connection) -> T): T = safe {
        checkPrincipal(principal); transactions.run { c -> authority.lockPrincipal(c, principal); action(c) }
    }
