-- Recognize retained moderator enrollment and moderation audit relations only.
-- Preserve every V076 account-core delete scope and related-data hold unchanged.
-- A narrowly linked immutable staff event is not public evidence or account-owned
-- purge material. No moderation history is deleted and no serving grant is added.
...
DECLARE
 ...
 fenced_tables constant text[]:=ARRAY[
   ...
   'platform.account_notifications','platform.notification_read_watermarks',
   ...
   'staff.moderator_enrollments','safety.moderation_access_audit','safety.moderation_actions'];
...
BEGIN
 ...
 IF EXISTS(SELECT 1 FROM platform.account_notifications n WHERE n.environment=p_environment
      AND (n.recipient_user_id=accepted.user_id OR n.sender_user_id=accepted.user_id)) THEN
  RETURN 'related_data';
 END IF;
