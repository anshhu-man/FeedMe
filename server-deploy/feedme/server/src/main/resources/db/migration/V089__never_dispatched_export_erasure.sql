-- Erase only exact-account export jobs that never crossed the durable upload-dispatch boundary.
-- Any write-attempted, verified or otherwise dispatched artifact remains a related-data hold;
-- expiry, key clearing and a cleanup attempt do not prove external absence. Export cleanup
-- stays inside the existing accepted-job, live-lease, fixed-inventory core transaction.
 ...
 RETURN EXISTS(
  ...
  WHERE s.backend_pid=pg_backend_pid() AND s.transaction_id=txid_current()
    ...
    AND ((p_relation IN ('profile.onboarding_decisions'::regclass,
      ...
      OR (p_relation IN ('platform.account_export_jobs'::regclass,'platform.account_export_artifacts'::regclass)
          AND p_account_id=s.account_id AND p_principal_id=s.principal_id))
 );
END;
$feedme_core_allowed$;
REVOKE ALL ON FUNCTION identity.account_erasure_delete_allowed(oid,text,uuid,uuid) FROM PUBLIC;

CREATE OR REPLACE FUNCTION platform.guard_account_export_jobs() RETURNS trigger LANGUAGE plpgsql
SET search_path=pg_catalog,pg_temp AS $feedme_export_jobs$
BEGIN
 IF TG_OP='DELETE' AND identity.account_erasure_delete_allowed(
      TG_RELID,OLD.environment,OLD.account_id,OLD.principal_id) THEN RETURN OLD; END IF;
...
CREATE OR REPLACE FUNCTION platform.guard_account_export_artifacts() RETURNS trigger LANGUAGE plpgsql
...
DECLARE
 ...
 account_ops constant text[]:=ARRAY['bootstrapAccount','logoutSession','revokeSession','acceptAccountTerms','updateMe','requestAccountDeletion',
   ...
   'markNotificationRead','markNotificationsRead','setReaction','removeReaction','requestAccountExport'];
 ...
 account_events constant text[]:=ARRAY['identity.account.bootstrapped.v1','identity.session.revoked.v1',
   ...
   'social.reaction.changed.v1','social.reaction.removed.v1','identity.account.export_requested.v1'];
...
BEGIN
 ...
 IF EXISTS(SELECT 1 FROM platform.media_digest_readiness d WHERE d.environment=p_environment
  ...
  RETURN 'related_data';
 END IF;
 -- Only never-dispatched exact-owner exports are local private data in this slice.
 -- A running lease without a dispatch marker is safe: the fixed table locks below
 -- serialize against markDispatched, while accepted deletion prevents a new claim.
 -- Any attempted/verified dispatch remains held even after expiry or key clearing.
 IF EXISTS(SELECT 1 FROM platform.account_export_jobs e WHERE e.environment=p_environment
     ...
     AND ROW(e.principal_id,e.issuer,e.provider_subject)
       IS DISTINCT FROM ROW(accepted.principal_id,accepted.provider_issuer,accepted.provider_subject))
   OR EXISTS(SELECT 1 FROM platform.account_export_artifacts e WHERE e.environment=p_environment
     AND e.account_id=accepted.user_id AND e.principal_id<>accepted.principal_id)
   OR EXISTS(SELECT 1 FROM platform.account_export_artifacts e WHERE e.environment=p_environment
     AND e.account_id=accepted.user_id
     AND (e.write_attempted_at IS NOT NULL OR e.dispatch_token IS NOT NULL
       OR e.dispatch_generation IS NOT NULL OR e.verified_at IS NOT NULL)) THEN
  RETURN 'related_data';
 END IF;
 ...
 -- Every export job must retain its exact command identity, and a completed
 -- response must be the bounded request acknowledgement for that same job.
 -- Pending/tombstoned receipts with no job are local command state and may erase.
 IF EXISTS(SELECT 1 FROM platform.account_export_jobs j
     WHERE j.environment=p_environment AND j.account_id=accepted.user_id
       AND j.principal_id=accepted.principal_id
       AND NOT EXISTS(SELECT 1 FROM platform.idempotency i
         WHERE i.principal_scope=account_scope AND i.operation_id='requestAccountExport'
           AND i.key=j.command_key AND lower(i.request_hash)=j.request_sha256))
   OR EXISTS(SELECT 1 FROM platform.idempotency i
     WHERE i.principal_scope=account_scope AND i.operation_id='requestAccountExport'
       AND i.state='completed'
       AND NOT EXISTS(SELECT 1 FROM platform.account_export_jobs j
         WHERE j.environment=p_environment AND j.account_id=accepted.user_id
           AND j.principal_id=accepted.principal_id AND j.command_key=i.key
           AND j.request_sha256=lower(i.request_hash) AND i.response_code=202
           AND i.response_etag IS NULL AND jsonb_typeof(i.response_json)='object'
           AND i.response_json->>'jobId'=j.id::text
           AND i.response_json->>'status'='pending'
           AND jsonb_typeof(i.response_json->'pollAfterSeconds')='number'
           AND (i.response_json->>'pollAfterSeconds')::numeric BETWEEN 1 AND 60
           AND trunc((i.response_json->>'pollAfterSeconds')::numeric)=(i.response_json->>'pollAfterSeconds')::numeric
           AND NOT EXISTS(SELECT 1 FROM jsonb_object_keys(i.response_json) AS response_keys(key)
             WHERE response_keys.key NOT IN ('jobId','status','pollAfterSeconds')))) THEN
 ...
 -- Export request events are removable only with their exact retained job source.
 -- A typed event without this closed lineage is unknown history, not an owned fact.
 IF EXISTS(SELECT 1 FROM platform.account_export_jobs j
     WHERE j.environment=p_environment AND j.account_id=accepted.user_id
       AND j.principal_id=accepted.principal_id
       AND 1<>(SELECT count(*) FROM platform.outbox e
         WHERE e.event_type='identity.account.export_requested.v1' AND e.schema_version=1
           AND e.aggregate_type='account_export' AND e.aggregate_id=j.id AND e.aggregate_version=1
           AND e.producer='identity' AND e.causation_id=j.command_key
           AND e.correlation_id=j.command_key::text
           AND e.owner_environment=p_environment AND e.owner_kind='account'
           AND e.owner_id=accepted.user_id
           AND e.payload=jsonb_build_object('userId',accepted.user_id::text,
             'jobId',j.id::text,'format',j.format,'includeMedia',j.include_media)))
   OR EXISTS(SELECT 1 FROM platform.outbox e
     WHERE e.owner_environment=p_environment AND e.owner_kind='account'
       AND e.owner_id=accepted.user_id
       AND e.event_type='identity.account.export_requested.v1'
       AND NOT EXISTS(SELECT 1 FROM platform.account_export_jobs j
         WHERE j.environment=p_environment AND j.account_id=accepted.user_id
           AND j.principal_id=accepted.principal_id AND e.schema_version=1
           AND e.aggregate_type='account_export' AND e.aggregate_id=j.id AND e.aggregate_version=1
           AND e.producer='identity' AND e.causation_id=j.command_key
           AND e.correlation_id=j.command_key::text
           AND e.payload=jsonb_build_object('userId',accepted.user_id::text,
             'jobId',j.id::text,'format',j.format,'includeMedia',j.include_media))) THEN
 ...
 IF EXISTS(SELECT 1 FROM platform.outbox e WHERE e.owner_environment=p_environment AND e.owner_kind='account'
   ...
   AND ((e.event_type='safety.block.changed.v1' AND (e.payload->>'blockerUserId'=accepted.user_id::text OR e.payload->>'blockedUserId'=accepted.user_id::text))
     ...
     OR (e.event_type='identity.account.export_requested.v1'
       AND (e.payload->>'userId'=accepted.user_id::text
         OR EXISTS(SELECT 1 FROM platform.account_export_jobs j
           WHERE j.environment=p_environment AND j.account_id=accepted.user_id
             AND (e.aggregate_id=j.id OR e.payload->>'jobId'=j.id::text))))
     OR (e.event_type='conversations.message.created.v1' AND e.payload->>'senderUserId'=accepted.user_id::text))) THEN
  RETURN 'related_data';
 END IF;
 IF EXISTS(SELECT 1 FROM platform.outbox e WHERE e.owner_environment=p_environment
   AND e.event_type='identity.account.export_requested.v1'
   AND ROW(e.owner_kind,e.owner_id) IS DISTINCT FROM ROW('account'::text,accepted.user_id)
   AND (e.payload->>'userId'=accepted.user_id::text
     OR EXISTS(SELECT 1 FROM platform.account_export_jobs j
       WHERE j.environment=p_environment AND j.account_id=accepted.user_id
         AND (e.aggregate_id=j.id OR e.payload->>'jobId'=j.id::text)))) THEN
  RETURN 'related_data';
 END IF;
 ...
 -- Children first. Guard exceptions require both accepted account and private
 -- principal coordinates from this exact transaction-local scope.
 DELETE FROM platform.account_export_artifacts a
  WHERE a.environment=p_environment AND a.account_id=accepted.user_id
    AND a.principal_id=accepted.principal_id;
 DELETE FROM platform.account_export_jobs j
  WHERE j.environment=p_environment AND j.account_id=accepted.user_id
    AND j.principal_id=accepted.principal_id;
