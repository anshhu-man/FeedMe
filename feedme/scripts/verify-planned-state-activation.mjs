...
// Private-data activation planning/selection only; no setup-journal, abort or durable replay claim.
// Requires existing JDK17, SDK36, local PostgreSQL binaries and a booted local emulator.
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const destination = path.join(root, 'docs/verification/planned-state-activation');
...
const report = {
  ...
  scope: 'M1.05d.5b.2b.1 only: MAC-authenticated private-data activation plans bind an exact absent or retired predecessor, next generation and preselected native key ID before owner-key/SQLite selection writes. Planning is read-only; commit returns Unit without a private-data handle, validates exact key references and never recreates a missing selected key or automatically erases interrupted material. Selected replay acknowledges the current exact selection, preserves existing records and makes no fresh fsync or power-loss durability guarantee. The low-level caller must durably retain one exact pending plan per predecessor; PrivateSessionRuntime is not wired to data planning in this slice. No independent private-data setup-journal resolution, existing-only data-plan abort/opener, consumed-plan cleanup, whole-session crash-safe creation or planned work-origin integration is claimed. Fresh shared/server/isolated-PostgreSQL/Node regressions, five Android library builds/lint and isolated native storage, credential-plan, cancellation and runtime/control integration suites. Real SQLite/Keystore tests cover injected key/SQL failures and close/reopen, not physical-device, process-hard-kill-in-transaction or power-loss proof; only the historical storage restart pair uses separate instrumentation processes. Identity verification remains fake or synthetic, not provider or UI proof. Credential API 26 remains NOT_CONFIGURED. Hardlink and notification branches are retained explicitly; denied construction is not execution of the existing-hardlink guard, and unavailable permission is not delivered-notification cancellation. No app recovery/confirmation wiring, provider sign-in, generic orphan reset, iOS or release claim.',
