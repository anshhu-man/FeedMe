...
/** Real existing-only native control storage SPI; no initialization, nested-plan authority or startup consent. */
@OptIn(SessionControlRecoveryCompositionApi::class)
@RunWith(AndroidJUnit4::class)
class AndroidSessionControlRecoveryStoreTest {
    private val fixtures = mutableListOf<NativeControlFixture>()
    private val emptyBoxes = mutableListOf<AndroidStateTestSandbox>()
    private val stores = mutableListOf<ExistingSessionControlRecoveryStore>()
    private val faults = mutableListOf<Faults>()
    private val terminals = mutableListOf<Pair<ExistingSessionControlRecoveryStore, AndroidStateTestSandbox>>()
    ...
    @Test fun constructionDefersContextIoAndCloseBeforeOpenNeverInitializesMissingStorage() = runBlocking {
        ...
        val owner = AndroidSessionControlStore.createRecoveryStore(trap).also(stores::add)
        assertEquals(0, locations); assertEquals("ExistingSessionControlRecoveryStore(<redacted>)", owner.toString())
    ...
    @Test fun existingIdlePendingCompleteAndOpaqueRecordsAreReadWithoutWritingOrSigning() = runBlocking {
        for (stage in listOf("idle", "pending", "complete", "opaque")) {
            ...
            assertFalse(owner is WorkOriginPlanAuthentication); assertFalse(owner is WorkOriginPlanVerification)
            ...
            assertEquals(before, snapshot(f.native.box)); assertEquals(keys, f.native.aliases())
        }
    }

    @Test fun fixedControlOwnerCannotAdoptExistingWorkLedgerOrForeignControlCiphertext() = runBlocking {
        val workBox = AndroidStateTestSandbox().also(emptyBoxes::add)
        val work = AndroidStateDatabase.openOwned(
            location = { workBox.directory to workBox.keyPrefix },
            requireNewVaultForNewFile = true, requireNewDirectoryForInitialization = true,
            wrap = { database, created -> EncryptedSessionWorkStore.open(database, created) },
        ).valueOrFail()
        work.close().valueOrFail()
        val workBefore = snapshot(workBox); val workKeys = aliases(workBox)
        val wrongPurpose = owner(workBox)
        failure(wrongPurpose.open(), FailureReason.STORAGE_FAILURE); unavailable(wrongPurpose); close(wrongPurpose)
        assertEquals(workBefore, snapshot(workBox)); assertEquals(workKeys, aliases(workBox))

        val first = fixture(); val other = fixture()
        val source = File(first.native.box.directory, "state.sqlite")
        val target = File(other.native.box.directory, "state.sqlite")
        target.writeBytes(source.readBytes()) // Exact closed, owned ciphertext only; keys stay separate.
        val before = snapshot(other.native.box); val keys = other.native.aliases()
        val foreign = owner(other.native.box)
        failure(foreign.open(), FailureReason.STORAGE_FAILURE); unavailable(foreign); close(foreign)
        assertEquals(before, snapshot(other.native.box)); assertEquals(keys, other.native.aliases())
    }

    @Test fun exactSamePayloadCasChangesRevisionAndSurvivesExistingOnlyReopen() = runBlocking {
        val f = fixture("pending"); val keys = f.native.aliases()
        val owner = owner(f.native.box); owner.open().valueOrFail()
        ...
        close(reopened); assertEquals(keys, f.native.aliases())
    }
    ...
    @Test fun cancelledAcquisitionAndCallerReturnGapNeverExposeLedgerOrCasAuthority() = runBlocking {
        for (stage in OPEN_STAGES) {
            ...
            assertEquals(before, snapshot(box))
        }
        for (closeBeforeReturn in listOf(false, true)) {
            ...
            unavailable(owner); close(owner); assertEquals(before, snapshot(box))
        }
    }
    ...
    private class Fixture(val native: NativeControlFixture, val current: SessionControlRecord)
    private suspend fun fixture(stage: String = "idle"): Fixture {
        val native = NativeControlFixture().also(fixtures::add)
        val current = native.initialize(stage)
        native.close()
        return Fixture(native, current)
    }

    private class NativeControlFixture {
        val box = AndroidStateTestSandbox()
        private var control: EncryptedSessionControlStore? = null
        suspend fun initialize(stage: String): SessionControlRecord {
            val owned = AndroidStateDatabase.openOwned(
                location = { box.directory to box.keyPrefix },
                requireNewVaultForNewFile = true, requireNewDirectoryForInitialization = true,
                wrap = { database, created -> EncryptedSessionControlStore.open(database, created) },
            ).valueOrFail().also { control = it }
            val initial = checkNotNull(owned.read().valueOrFail())
            // Control storage treats bytes as bounded opaque metadata. Session codecs alone
            // validate real pending/complete records; these labels are not forged session plans.
            return if (stage == "idle") initial else
                owned.compareAndSet(initial.revision, bytes("opaque-control-fixture-$stage")).valueOrFail()
        }
        suspend fun close() {
            control?.let { it.close().valueOrFail(); control = null }
        }
        fun destroy() = box.close()
        fun aliases() = box.keyStore().aliases().toList().filter { it.startsWith("${box.keyPrefix}.") }.toSet()
    }
    ...
    private fun owner(box: AndroidStateTestSandbox, fault: Faults = fault()) =
        AndroidSessionControlStore.createRecoveryStoreForTests(box.directory, box.keyPrefix, fault).also(stores::add)
    private suspend fun close(owner: ExistingSessionControlRecoveryStore) { owner.close().valueOrFail(); stores.remove(owner) }
    private suspend fun unavailable(owner: ExistingSessionControlRecoveryStore) {
    ...
    companion object {
        private const val PRIVATE_FAULT = "private-native-control-recovery-fixture"
