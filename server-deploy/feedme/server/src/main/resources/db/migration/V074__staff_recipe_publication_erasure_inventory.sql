-- Recognize retained qualification evidence and catalog publication/recall command history.
-- Existing staff holds remain; no deletion, grants or publication is performed.
-- Recognize immutable staff submissions and editorial decisions in the erasure inventory.
-- Existing staff-related holds remain; no review history is deleted and no grants are added.
-- Recognize unpublished staff draft history in the reviewed relation inventory.
-- Existing staff actor/approval related-data holds remain; no draft deletion or grants.
-- Account Make Again is private owner linkage, deleted before either referenced child.
-- Preserve V065 exact export/shared-history holds; no cloud erasure or grants.
