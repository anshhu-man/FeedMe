...
Basic app-recipe persistence additionally binds environment/actor kind to principal ownership and normalizes dedup lineage by reviewed recipe version plus exact materialized snapshot hash. Distinct reviewed scaling remains distinct; deletion tombstones one saved identity and re-save creates a new generation/ID. Item cursors bind the exact owner, collection/version and stable position/ID. Canonical Collection.nextItemCursor pages IDs independently from CollectionPage.nextCursor; no bounded page is a total-save quota. Enhanced/custom organization remains deferred to F45.
