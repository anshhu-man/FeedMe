...
| M1.05d.5b.2b.2 | IN_PROGRESS | [Planned state recovery](PLANNED_STATE_RECOVERY.md) code implements existing-only inspection and empty-only abort with a changed V2 receipt COMMIT before exact key deletion; 315 targeted storage JVM tests pass, full/native verification pending |
| M1.05d.5b.2b.3 | TODO | Empty work-origin planning and one independent composite setup journal retaining every plan through binding persistence, acknowledged durable completion and final lease activation |
...
Readback can use SQLite or operating-system caches. Therefore, as an engineering inference, a matching row alone is not evidence that a genuinely failed synchronization has been repaired. The historical selection tests below do not establish that this driver exposes such a matching-but-undurable state; they also do not rule it out. The [recovery continuation](PLANNED_STATE_RECOVERY.md) implements a changed-row acknowledgement and a native syscall-failure test seam; its full/native verification is pending. No additional-descriptor redesign is selected by this component. [SQLite cache retention](https://www.sqlite.org/atomiccommit.html#cache_retained_between_transactions).
...
Current continuation: [planned state recovery](PLANNED_STATE_RECOVERY.md), **IN_PROGRESS**. The targeted storage JVM run passes **315 tests**, including 40 new recovery cases; full source-bound/native verification is pending. Do not treat the historical receipt below as proof of the changed V2 recovery sources.
...
Reproduce with JDK17, the installed Android SDK, PostgreSQL 15 binaries and a booted API35 emulator:
