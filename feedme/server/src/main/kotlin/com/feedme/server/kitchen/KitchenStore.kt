...
class KitchenStore(private val environment: String, private val transactions: PgTransactions,
    ...
    fun provisionPreferences(actor: VerifiedKitchenPrincipal, initial: JsonObject): StoredReply {
        ...
        return read(actor) { c ->
            ...
            val existing = preference(c, actor)
            if (existing != null) {
                if (existing.fields != fields) fail(KitchenFailureCode.VERSION_CONFLICT)
                val result = preferenceJson(existing)
                reply("getPreferences", 200, result, existing.version)
            } else {
                val id = UUID.randomUUID(); val at = now(c)
                val row = Row(id, 1, at, at, fields)
                val result = preferenceJson(row)
                reply("getPreferences", 200, result, 1)
                authority.validatePreferences(c, actor, result)
                exec(c, "INSERT INTO profile.preferences(environment,actor_kind,principal_id,id,version,created_at,updated_at,fields) VALUES(?,?,?,?,1,?,?,?::jsonb)") {
                    owner(actor); setObject(4, id); setObject(5, time(at)); setObject(6, time(at)); setString(7, fields.toString())
                }
                preferenceEvent(c, actor, UUID.randomUUID(), row, fields.keys)
                reply("getPreferences", 200, preferenceJson(preference(c, actor)!!), 1)
            }
        }
    }

    fun getPreferences(actor: VerifiedKitchenPrincipal): StoredReply = read(actor) { c ->
    ...
    fun updatePreferences(actor: VerifiedKitchenPrincipal, key: UUID, ifMatch: String, body: JsonObject): CommandResult {
        ...
        return command(actor, "updatePreferences", key, body = patch, ifMatch = ifMatch, replay = { c, cached ->
            val row = preference(c, actor) ?: fail(KitchenFailureCode.PREFERENCES_UNAVAILABLE)
    ...
    fun upsertPantryItem(actor: VerifiedKitchenPrincipal, key: UUID, body: JsonObject): CommandResult {
        ...
        return command(actor, "upsertPantryItem", key, body = input, replay = { c, cached ->
    ...
    fun removePantryItem(actor: VerifiedKitchenPrincipal, key: UUID, ingredientId: UUID, ifMatch: String): CommandResult {
        ...
        return command(actor, "removePantryItem", key, paths = mapOf("ingredientId" to ingredientId.toString()), ifMatch = ifMatch, replay = { c, cached ->
    ...
    private fun command(actor: VerifiedKitchenPrincipal, op: String, key: UUID, paths: Map<String, String> = emptyMap(),
        ...
        commands.execute(command, { authority.lockPrincipal(it, actor) }, {}, replay, mutate)
    }
    private fun <T> read(actor: VerifiedKitchenPrincipal, action: (Connection) -> T): T = safe {
        checkActor(actor); transactions.run { c -> authority.lockPrincipal(c, actor); action(c) }
    }
