    ...
    fun execute(
        ...
        """.trimIndent()).use { statement ->
            ...
            statement.executeQuery().use { row ->
                ...
                if (row.getString("request_hash") != command.requestHash) return@run CommandResult.Mismatch
                if (!inserted) {
                    if (row.getString("state") == "tombstone") return@run CommandResult.ReceiptExpired
                    if (row.getString("state") != "completed") return@run CommandResult.IncompleteReceipt
                    if (expireLockedReceipt(connection, command)) {
                        return@run CommandResult.ReceiptExpired
                    }
                    ...
                    if (expireLockedReceipt(connection, command)) return@run CommandResult.ReceiptExpired
                    return@run CommandResult.Replayed(reply)
                }
            }
        }
