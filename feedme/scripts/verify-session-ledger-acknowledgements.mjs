...
// Independent session/control/work changed-CAS acknowledgements, with exact native VFS evidence.
// Frozen inventory: 1,146 Kotlin/server/PostgreSQL, 102 Node and 131 native checks.
// Requires existing JDK17, SDK36, local PostgreSQL binaries and a booted local emulator.
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const destination = path.join(root, 'docs/verification/session-ledger-acknowledgements');
...
const report = {
  ...
  scope: 'M1.05d.5b.2b.3.1 component only: independent control/work writes must obtain this invocation\'s acknowledged changed CAS, not promote matching readback after an uncertain commit into create, cleanup or publication authority. Exact retry preserves the original payload and identity, checks cancellation/current ownership, and acknowledges the freshly observed revision again. Nine tests in a separate instrumentation process inject VFS sync errors into the actual bundled SQLite engine for credential-abort control barriers, work reservations and work-retirement barriers; each verifies that its downstream native effect did not run. These use an explicitly URI-selected registered forwarding VFS, not OS errno failures. Six existing private-data-abort VFS cases remain separate. Direct owned-connection fixtures may permit SQLite rollback-journal recovery and do not prove the strict public recovery factory accepts journals. Fresh shared/server/isolated-PostgreSQL/Node regressions, five Android library builds/lint, 74 native storage and 57 native session tests; four-ABI JNI test helper provenance and absence from production artifacts remain checked. Hardlink and notification branch limits remain explicit; identity verification is synthetic. Private activation-binding OUTCOME_UNKNOWN/readback durability remains the separate b.2b.3.5 gate; these independent-ledger results do not resolve it. Composite pending-setup codec, work-origin planning, exact binding/publication integration, provider/bootstrap/UI, first-factory repair, physical power-loss/hard-kill, physical-device, API26 recovery, iOS and release acceptance remain open. No new deployment or publication action.',
...
const nativeSuites = [
  {
    ...
    script: 'scripts/android-storage-smoke.mjs', cleanupPattern: /private-state-instrumented-|feedme-state-instrumented-process-restart-v1/,
    checks: [
...
try {
  ...
  if (!/^# tests 102$/m.test(nodeLog) || !/^# pass 102$/m.test(nodeLog) || !/^# fail 0$/m.test(nodeLog) || !/^# skipped 0$/m.test(nodeLog))
    throw new Error('Expected all 102 Node regressions without skips.');
  for (const [directory, name, expected] of [
    ...
    ['shared/storage/build/test-results/jvmTest', 'storage', 323],
    ['shared/sync/build/test-results/jvmTest', 'sync', 103],
    ['shared/kitchen/build/test-results/jvmTest', 'kitchen', 142],
    ['shared/session/build/test-results/jvmTest', 'session', 248],
  ...
  report.node = {tests: 102, failures: 0, errors: 0, skipped: 0};
  report.androidLint = [];
