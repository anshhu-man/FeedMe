...
// Ordered live composite selection only; retained journal is not setup completion.
...
const destination = path.join(root, 'docs/verification/live-session-setup');
...
const report = {
  ...
  scope: 'M1.05d.5b.2b.3.4 bounded internal live selection coordinator and read-only data-plan inspection only: prepare credential CREATE, private-data activation and work-origin plans without effects; obtain a successful changed-CAS acknowledgement and exact readback of their single composite journal before selecting those exact resources in order. The same in-memory verified attempt retains its exact plans through supported retries, with pending control rechecked before each downstream selection. Failures or cancellation retain PendingSetup and authorize no later resource selection. Live data inspection authenticates the exact plan/scope and observes metadata only, never key usability, record integrity or durability. Selected data replay performs no changed SQL write and is not a fresh durability acknowledgement; retaining the journal must not be confused with completing setup. Native credential mutation poisoning, partial-inventory ordinary-open refusal and loss of live credential authority on restart remain explicit limitations, not automatic repair or resumed sign-in. No runtime integration, real provider verification, active lease, private activation binding, work-origin sealing, composite abort or completion/journal-clear authority is implemented by this slice. The new live-coordinator fault wrappers model application acknowledgement loss after successful native calls or blocked control/readback, not additional native sync failures. All prior component, five separate-process stages and actual-engine VFS regressions remain included; the 21 identity-bound VFS labels report injected SQLite errors, not OS errno failures or physical power loss, and direct owned-connection fixtures do not prove public hot-journal recovery. Trusted typed configuration/digest production, cross-store native preflight for recovery, provider/bootstrap/UI, first-factory repair, physical devices, API26 recovery, iOS and release acceptance remain open. Fresh shared/server/isolated PostgreSQL/Node regressions, five Android library builds/lint, and test-only four-ABI JNI provenance are included. The historical demo APK and public GitHub snapshot are unchanged; no new deployment or publication.',
...
try {
  ...
  for (const [directory, name, expected] of [
    ...
    ['shared/storage/build/test-results/jvmTest', 'storage', 370],
    ['shared/sync/build/test-results/jvmTest', 'sync', 103],
    ['shared/kitchen/build/test-results/jvmTest', 'kitchen', 142],
    ['shared/session/build/test-results/jvmTest', 'session', 351],
