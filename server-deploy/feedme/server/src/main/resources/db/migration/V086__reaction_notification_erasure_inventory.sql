-- V085 reaction notification receipts are shared actor/recipient history.
-- Add a finite related-data hold only; preserve all V084 purge predicates.
...
DECLARE
 ...
 unsupported_ops constant text[]:=ARRAY['createCircle','updateCircle','updateCircleMember','leaveCircle','removeCircleMember',
   ...
   'publishPost','updatePost','deletePost','setReaction','removeReaction','blockUser','unblockUser','createReport','createThread','sendMessage','markThreadRead',
...
BEGIN
 ...
 -- Inbox receipts retain a real peer/message association. Recognize exact ownership,
 -- but do not invent shared-history deletion, remove another recipient's receipt, or
 -- infer that a read watermark authorizes erasure. Ordinary logout never deletes these.
 IF EXISTS(SELECT 1 FROM platform.account_notifications n WHERE n.environment=p_environment
   ...
   OR EXISTS(SELECT 1 FROM platform.account_reaction_notifications n WHERE n.environment=p_environment
      AND (n.recipient_user_id=accepted.user_id OR n.actor_user_id=accepted.user_id)) THEN
  RETURN 'related_data';
 END IF;
 ...
 -- Reaction history involves both the actor and the source author. Tombstones and
 -- pending cleanup remain related data; this stage gains no social purge authority.
 IF EXISTS(SELECT 1 FROM social.post_reactions r WHERE r.environment=p_environment
       AND (r.actor_user_id=accepted.user_id OR r.post_owner_user_id=accepted.user_id))
