...
const parsers = ['android-storage-smoke.mjs', 'verify-setup-abort-primitives.mjs'].map(name => {
  ...
  const declaration = source.match(/function bindingAbortSyncFailureEvidence\(log, identities\) \{[\s\S]*?\n\}/)?.[0];
  assert.ok(declaration, `${name} must retain the pure binding-abort evidence parser`);
  return vm.runInNewContext(`(${declaration})`, Object.create(null), {timeout: 1000});
});

const testClass = 'com.feedme.storage.AndroidStateBindingAbortSyncFailureTest';
const expected = [
...
const lines = expected.map(([label, , sqlite]) =>
  `INSTRUMENTATION_RESULT: data_binding_abort_sync_${label}=sqlite=${sqlite};failures=1;keyDeletes=0;vfs=1;delegatedUnlink=${sqlite === 1290 ? 1 : 0}`);
const log = lines.join('\n');
const rejects = (text, successful = identities) => {
  for (const parse of parsers) assert.throws(() => parse(text, successful));
};

test('binding-abort smoke and verifier agree on all six source-bound initial and replay VFS results', () => {
  const source = fs.readFileSync(fileURLToPath(new URL(
    '../shared/storage/src/androidInstrumentedTest/kotlin/com/feedme/storage/AndroidStateBindingAbortSyncFailureTest.kt',
  ...
  for (const [label, method, sqlite] of expected) assert.deepEqual(results[0][label], {
    ...
    failures: 1, keyDeletes: 0, vfs: 1, delegatedUnlink: sqlite === 1290 ? 1 : 0,
  });
});

test('binding-abort evidence requires both abort and replay labels for every sync stage', () => {
  for (let omitted = 0; omitted < lines.length; omitted++)
    rejects(lines.filter((_, index) => index !== omitted).join('\n'));
});

test('binding-abort evidence rejects duplicate labels even when the total count is still six', () => {
  rejects([...lines.slice(0, -1), lines[0]].join('\n'));
  rejects(`${log}\n${lines[0]}`);
});

test('binding-abort evidence rejects forged or additional result labels', () => {
  rejects(log.replace('initial-journal', 'select-unrecognized'));
  rejects(`${log}\n${lines[0].replace('initial-journal', 'unrecognized-recovery')}`);
});

test('binding-abort result bytes cannot substitute for a missing paired-success source identity', () => {
...
test('binding-abort evidence rejects successful identities borrowed from another class or method', () => {
  rejects(log, identities.map(identity => identity.replace('AndroidStateBindingAbortSyncFailureTest', 'AndroidSessionLedgerSyncFailureTest')));
  rejects(log, identities.map(identity => `${identity}Unrelated`));
});

test('binding-abort evidence rejects an incorrect SQLite extended error for the exact stage', () => {
...
test('binding-abort evidence requires one actual failure and zero native key deletions', () => {
  for (const replacement of ['failures=0', 'failures=2', 'failures=-1'])
    rejects(log.replace('failures=1', replacement));
  rejects(log.replace('keyDeletes=0', 'keyDeletes=1'));
});

test('binding-abort evidence preserves the explicit VFS mechanism and delegated-unlink stage', () => {
...
test('binding-abort evidence rejects truncated reordered or extra result fields', () => {
  rejects(log.replace(';keyDeletes=0', ''));
  rejects(log.replace('failures=1;keyDeletes=0', 'keyDeletes=0;failures=1'));
