...
// Isolated credential/storage components, not provider sign-in, actual app logout, or a release.
// Requires existing JDK17, SDK36, local PostgreSQL binaries and a booted local emulator.
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const destination = path.join(root, 'docs/verification/native-credentials');
...
const report = {
  ...
  scope: 'Incarnation-fenced native credential storage, strict private codecs and read-only transport projection; fresh shared/server/isolated-PostgreSQL/Node regressions, five Android library builds/lint, 65 isolated storage native tests, 19 credential native tests, 18 authenticated CREATE-plan native tests, 10 exact native-cancellation tests and ten separate real-store/runtime/diagnostic/setup/control integration tests. Runtime identity verification remains fake or synthetic, not an actual provider. Diagnostics observe already-open stores only, not native-factory failures, usable credentials or orphan repair. The current Android credential factory is gated to API 27+; API 26 NOT_CONFIGURED remains an implementation gate, not demonstrated API 26 support. Credential tests prove controlled close/reopen and injected file outcomes, not credential process restart. Hardlink fixture creation may be denied by the Android platform; this receipt makes no existing-hardlink-guard execution claim. Notification branch evidence distinguishes permission-unavailable from delivered cancellation. Storage alone includes two separate instrumentation-process stages. No production timer receiver/worker, app scheduling/recovery wiring, verified identity-provider sign-in/refresh, actual app UI/logout/account switch/deletion workflow, hard-kill-in-transaction, physical-device, power-loss, iOS or release claim.',
...
const nativeSuites = [
  ...
  {
    name: 'credentials', module: 'session', directory: 'native-credentials', package: 'com.feedme.session.test',
...
function retainNativeEvidence(suite, notBefore) {
  ...
  for (const check of suite.checks) {
    ...
    native.completedTests.push({name: check.name, identities: verifyInstrumentationTranscript(log, check)});
  }
