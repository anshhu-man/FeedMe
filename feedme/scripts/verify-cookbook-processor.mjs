...
// Focused ninth-batch checkpoint, not a rerun of the historical 354 native tests.
...
const fixed = {shared:2168, progress:45, server:250, postgresql:392, timers:25, timerOnly:6, host:6, timerHost:4, cookbookHost:4, processDeclarations:4, node:202};
...
async function main() {
  ...
  const base=path.join(root,'docs/verification/cookbook-processor'), attempt=path.join(base,'attempts',began.replaceAll(':','-'));
  ...
  const report={startedAt:began,passed:false,serial,attemptDirectory:path.relative(root,attempt),commands:[],tests:[],native:[],artifacts:[],androidLint:[],screenshots:[],
    scope:'Focused ninth-batch source-bound checkpoint: complete current shared/progress/server JVM and PostgreSQL/Node regressions; eight libraries and isolated progress app; actual foreground timer/cookbook controls and explicit synthetic progress identity/catalog/service. Configured media processing includes real bounded JPEG/PNG child decoding and PostgreSQL jobs/leases/write intents/cleanup exercised with explicitly synthetic object and malware/moderation adapters; no deployed worker, provider or accepting safety default. Native scope is25 timer +6 timer-only cancellation +6 progress host +4 actual timer host +4 cookbook host +2 fresh-process recoveries, totaling47 passed tests and2 separately witnessed interrupted starts with12 source-bound PNGs. Historical354 native tests and legacy VFS/process matrices are NOT rerun. Timer-only cancellation executes first in a fresh session-test UID and refuses WorkManager initialization. No real login/provider, deployment, complete F19/Make Again/social copy, signed offline manifest, physical power-loss, background timer/notification delivery, iOS or store-release acceptance. Only the explicitly supplied owned emulator is addressed; user preview5554 is rejected.'};
  ...
  try {
    ...
    need(report.screenshots.length===12&&new Set(report.screenshots.map(p=>p.name)).size===12,'Twelve exact source-bound screenshots required');
    // Fourteen successful host cases explicitly closed their known owners. Stop only this freshly
    ...
    need(report.native.length===7&&report.nativeTests.tests===47,'Only47 focused native passes in seven successful invocations allowed');
    const actualNative=report.native.flatMap(s=>s.identities),allDeclared=[...timerIds,...timerOnlyIds,...progressIds].sort();
    need(new Set(actualNative).size===47&&JSON.stringify([...actualNative,stageA,cookbookStageA].sort())===JSON.stringify(allDeclared),'Exact global native pass/interruption inventory mismatch');
