...
internal fun MediaProcessingSource.document(): JsonObject = buildJsonObject {
    ...
    put("objectVersion", objectVersionId); put("sha256", sha256); put("bytes", bytes); put("type", contentType)
...
internal fun processingSource(json: String): MediaProcessingSource {
    ...
    val v = MediaProcessingSource(MediaProcessingOwner(s("environment"), UUID.fromString(s("owner"))), UUID.fromString(s("media")),
        ...
        Instant.parse(s("acceptedAt")), s("key"), s("objectVersion"), s("sha256"), s("bytes").toLong(), s("type"), Instant.parse(s("deadline")))
...
internal fun cleanupTarget(c: Connection, o: MediaProcessingOwner, media: UUID, key: String, horizon: Instant, intent: UUID?) {
    px(c, "INSERT INTO platform.media_processing_cleanup(id,environment,owner_user_id,media_id,object_key,derivative_intent_id,not_before,available_at,state) VALUES(?,?,?,?,?,?,?,?,'pending') ON CONFLICT(object_key) DO NOTHING",
    ...
    pq(c, "SELECT environment,owner_user_id,media_id,derivative_intent_id,not_before FROM platform.media_processing_cleanup WHERE object_key=? FOR UPDATE", { setString(1, key) }) {
        ...
        if (it.getString("environment") != o.environment || it.getObject("owner_user_id", UUID::class.java) != o.ownerId ||
            ...
            (intent != null && pi(it, "not_before") != horizon)) processingFail(MediaProcessingFailureCode.CONFLICT)
    }
}
