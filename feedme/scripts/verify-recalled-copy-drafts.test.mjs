...
import {verifyTestDevice,historicalNinthClosure,verifyNodeTap,cookbookMarker,verifyCookbookNotice,verifyCookbookLineage,interrupted,verifyNative,
  instrumentEvents,marker,verifyScreenshotSources,cookbookHostClass,cookbookStageA,cookbookStageB,cookbookPngMethods,declarations} from './verify-recalled-copy-drafts.mjs';
...
test('recalled-copy checkpoint: both user previews and missing configuration are rejected before device access',()=>{
  assert.doesNotThrow(()=>verifyTestDevice('emulator-5556','/configured/sdk'));
  for(const serial of ['emulator-5554','emulator-5558',undefined,'','physical-device','emulator-5556;'])
    assert.throws(()=>verifyTestDevice(serial,'/configured/sdk'));
  for(const androidHome of [undefined,'','  ',42]) assert.throws(()=>verifyTestDevice('emulator-5556',androidHome));
});
test('recalled-copy checkpoint: cookbook marker retains exact positive Long revision and elapsed precision',()=>{
...
test('recalled-copy checkpoint: cookbook marker rejects oversized extra duplicate reordered and noncanonical fields',()=>{
...
test('recalled-copy checkpoint: cookbook marker rejects invalid identity hash numeric types and overflow',()=>{
...
test('recalled-copy checkpoint: cookbook notice requires all four exact bounded status fields including UID',()=>{
...
test('recalled-copy checkpoint: cookbook interrupted stage is live start only then exactly one non-success crash terminal',()=>{
...
test('recalled-copy checkpoint: cookbook recovery requires exact start success notice and unique normal terminal',()=>{
...
test('recalled-copy checkpoint: cookbook lineage rejects any changed content revision owner reused PID or stale elapsed time',()=>{
...
test('recalled-copy checkpoint: cookbook PNG inventory is tied to five literal calls in their actual source methods',()=>{
  ...
  assert.equal(cookbookPngMethods.length,5);
  assert.deepEqual(cookbookPngMethods.at(-1),{name:'progress-cookbook-recalled-delete-consent',
    identity: `${cookbookHostClass}#eRecalledDownloadedCopyRemovesWithRedactedConsentAndPreservesCooking`});
  assert.deepEqual(verifyScreenshotSources(source,cookbookHostClass,cookbookPngMethods),cookbookPngMethods);
  assert.throws(()=>verifyScreenshotSources(source.replace('screenshot("progress-cookbook-recalled-delete-consent")',''),cookbookHostClass,cookbookPngMethods));
...
test('recalled-copy checkpoint: cookbook process source declares only the exact interrupted and recovery identities',()=>{
...
test('recalled-copy checkpoint: historical parser closure and timer witness remain separate from current passes',()=>{
  ...
  const history=historicalNinthClosure();
  assert.equal(history.receipt.sha256,'840990ff363394aa3ae152616ba92e84659f9ab293452ee4c31e2c996df504ab');
  assert.equal(history.sourceManifestSha256,'6173cde4f7baaf2dcae6af2067c93fecebca719880851ba38c2ed2197ea793a7');
  assert.equal(history.files.length,9);
  const modules=history.files.filter(pair=>pair.source.path.startsWith('scripts/')).map(pair=>pair.source.path).sort();
  assert.deepEqual(modules,['scripts/verify-cookbook-processor.mjs','scripts/verify-cookbook-processor.test.mjs',
    'scripts/verify-native-cooking-host.mjs','scripts/verify-release-scope.mjs']);
  for(const pair of history.files){
    const bytes=fs.readFileSync(new URL('../'+pair.retained.path,import.meta.url));
    assert.equal(bytes.length,pair.source.bytes);assert.equal(createHash('sha256').update(bytes).digest('hex'),pair.source.sha256);
  }
  const historicalHost=history.files.find(pair=>pair.source.path.endsWith('/AndroidProgressCookbookHostTest.kt'));
  const oldSource=fs.readFileSync(new URL('../'+historicalHost.retained.path,import.meta.url),'utf8');
  assert.deepEqual(verifyScreenshotSources(oldSource,cookbookHostClass,cookbookPngMethods.slice(0,4)),cookbookPngMethods.slice(0,4));
  assert.throws(()=>verifyScreenshotSources(oldSource,cookbookHostClass,cookbookPngMethods));
  const tap='# Subtest: historical proof\nok 1 - historical proof\n# tests 1\n# pass 1\n# fail 0\n# cancelled 0\n# skipped 0\n# todo 0\n';
  assert.equal(verifyNodeTap(tap,['historical proof']).tests,1);
  assert.throws(()=>verifyNodeTap(tap,['current proof']));
  assert.throws(()=>verifyNodeTap(tap+'# tests 1\n',['historical proof']));
});
