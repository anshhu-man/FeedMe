...
// Source-bound existing-only work recovery; all-owning startup composition remains open.
...
const destination = path.join(root, 'docs/verification/work-recovery-owners');
...
const report = {
  ...
  scope: "M1.05d.5b.2b.3.6d.2 existing-only native work recovery: retained synchronous storage owner, fixed work owner/sole-ledger validation, native proof verification without signing, exact CAS and full revision observation. The public session facade pins the original scope/plan and inactive boundary, exposes only open/inspect/abort/close, and rejects normal-origin or changed provenance. Shared retained Android acquisition preserves private inventory, no-sidecar/no-CREATE opening, partial owners, truthful close acknowledgements and caller-handoff publication across private data and work. Work proof authentication necessarily follows guarded existing-only SQLite reads; it is not pre-SQLite authentication. No normal resume, initialization, migration, GC, new identity, scheduler effect or lease is supplied. The V1 manifest separately enforces the approved44 P1/10 deferred feature partition and retains shared resources; it is planning/build enforcement, not runtime feature gating or release certification. Earlier credential/data and borrowed-store composite-abort coverage remains. All-owning startup factory composition, owned-close-before-Complete coordination, integrated hard-kill recovery, provider/UI, physical-device/power-loss, public hot-journal, API26/iOS and release acceptance remain open. All39 prior VFS labels/five controlled process stages and isolated four-ABI helper checks remain; new ownership hook failures are not OS errno or new engine fault injection. No deployment, store release or publication is performed by this verifier.",
...
try {
  ...
  for (const [directory, name, expected] of [
    ...
    ['shared/storage/build/test-results/jvmTest', 'storage', 518],
