...
// Focused tenth-batch checkpoint, not a rerun of the historical 354 native tests.
...
const fixed = {shared:2218, progress:56, server:283, postgresql:441, timers:25, timerOnly:6, host:6, timerHost:4, cookbookHost:5, processDeclarations:4, node:213, currentNode:203, historicalNode:10};
const ninthReceipt = 'docs/verification/cookbook-processor/attempts/2026-09-14T08-50-11.189Z/report.json';
const ninthReceiptSha256 = '840990ff363394aa3ae152616ba92e84659f9ab293452ee4c31e2c996df504ab';
const ninthParserTest = 'scripts/verify-cookbook-processor.test.mjs';
...
const cookbookHostClass = `${progressPackage}.AndroidProgressCookbookHostTest`;
const cookbookProcessClass = `${progressPackage}.AndroidProgressCookbookProcessTest`;
const cookbookStageA = `${cookbookProcessClass}#stageARetainAcknowledgedSavedRecipeThenAwaitExternalProcessStop`;
const cookbookStageB = `${cookbookProcessClass}#stageBRestoreExactSavedRecipeAndExistingServiceThenExplicitlyReset`;
const cookbookPngMethods = [
  {name:'progress-cookbook-saved',identity:`${cookbookHostClass}#aExplicitSaveShowsExactMaterializedRecipeThenDistinctRemotePage`},
  {name:'progress-cookbook-page',identity:`${cookbookHostClass}#aExplicitSaveShowsExactMaterializedRecipeThenDistinctRemotePage`},
  {name:'progress-cookbook-restored',identity:`${cookbookHostClass}#bActivityRecreationAndAcknowledgedNativeCloseResumeRetainSameDownloadedCopy`},
  {name:'progress-cookbook-delete-consent',identity:`${cookbookHostClass}#cExactRemovalConsentDoesNotEraseExistingCookingPin`},
  {name:'progress-cookbook-recalled-delete-consent',identity:`${cookbookHostClass}#eRecalledDownloadedCopyRemovesWithRedactedConsentAndPreservesCooking`},
];
...
function sourceFiles() {
  const files = ['build.gradle.kts','settings.gradle.kts','gradle.properties','gradlew','gradlew.bat',
    ...
    // Historical parser replay is a separate source authority, never relabeled current-source.
    ninthReceipt,...historicalNinthClosure().files.map(pair=>pair.retained.path)];
  need(new Set(files).size === files.length, 'Duplicate source path'); return files.sort().map(descriptor);
}
/** Complete reviewed file/dependency closure actually read by the immutable ninth tests.
 * The unchanged modules have entry-point guards, so imports do not execute their verifiers.
 * Their only non-module reads are the cookbook screenshot source and a walk of this five-file
 * Android test subtree. Preserve the original relative workspace paths; never patch assertions.
 */
function historicalNinthClosure() {
  const receipt=descriptor(ninthReceipt);need(receipt.sha256===ninthReceiptSha256,'Historical ninth receipt changed');
  const baseline=JSON.parse(read(ninthReceipt));
  need(baseline.passed===true&&baseline.startedAt==='2026-09-14T08:50:11.189Z'&&baseline.finishedAt==='2026-09-14T09:00:46.781Z'&&
    Array.isArray(baseline.sourceFiles)&&Array.isArray(baseline.frozenSources)&&
    baseline.sourceManifestSha256===hash(JSON.stringify(baseline.sourceFiles)),'Unverified ninth source authority');
  const scripts=['scripts/verify-cookbook-processor.mjs',ninthParserTest,'scripts/verify-native-cooking-host.mjs','scripts/verify-release-scope.mjs'];
  const directory='apps/android/src/androidTestProgress/';
  const kotlin=['AndroidProgressCookbookHostTest','AndroidProgressCookbookProcessTest','AndroidProgressHostTest','AndroidProgressProcessTest','AndroidProgressTimerHostTest']
    .map(name=>`${directory}kotlin/com/feedme/development/progress/${name}.kt`);
  const expected=[...scripts,...kotlin].sort();
  const files=baseline.frozenSources.filter(pair=>scripts.includes(pair.source?.path)||pair.source?.path?.startsWith(directory));
  need(JSON.stringify(files.map(pair=>pair.source.path).sort())===JSON.stringify(expected),'Historical dependency/source closure changed');
  for(const pair of files){
    const source=baseline.sourceFiles.filter(item=>item.path===pair.source.path);
    need(source.length===1&&source[0].bytes===pair.source.bytes&&source[0].sha256===pair.source.sha256&&
      pair.retained.path.startsWith(path.dirname(ninthReceipt)+'/sources/')&&
      /^\d{4}-[^/]+$/.test(path.basename(pair.retained.path))&&
      path.basename(pair.retained.path).slice(5)===path.basename(pair.source.path),'Invalid ninth source binding');
    const retained=descriptor(pair.retained.path);
    need(retained.bytes===pair.retained.bytes&&retained.sha256===pair.retained.sha256&&
      retained.bytes===pair.source.bytes&&retained.sha256===pair.source.sha256,'Historical closure copy hash mismatch');
  }
  return {receipt,sourceManifestSha256:baseline.sourceManifestSha256,files};
}
...
function nodeDeclarations(files, expected = fixed.node) {
  const ids = [];
  ...
  need(ids.length === expected && new Set(ids).size === ids.length, 'Node inventory changed'); return ids.sort();
}
function verifyNodeTap(tap, identities) {
  const starts=[...tap.matchAll(/^# Subtest: (.*)$/gm)].map(m=>m[1]),passed=[...tap.matchAll(/^ok (\d+) - (.*)$/gm)];
  need(JSON.stringify(starts.sort())===JSON.stringify(identities)&&JSON.stringify(passed.map(m=>m[2]).sort())===JSON.stringify(identities)&&
    passed.every((m,i)=>Number(m[1])===i+1)&&!/^not ok\b|# (?:SKIP|TODO)\b/m.test(tap),'Exact Node TAP identity mismatch');
  for(const [key,count]of Object.entries({tests:identities.length,pass:identities.length,fail:0,cancelled:0,skipped:0,todo:0})){
    const found=[...tap.matchAll(new RegExp(`^# ${key} (\\d+)$`,'gm'))];need(found.length===1&&Number(found[0][1])===count,'Node TAP count mismatch');
  }
  return {tests:identities.length,identities,failures:0,cancelled:0,skipped:0,todo:0};
}
...
function instrumentEvents(output) {
  ...
  for (const line of output.split(/\r?\n/)) {
    const f = line.match(/^INSTRUMENTATION_STATUS: (class|test|id|current|numtests|feedme_progress_stage|feedme_progress_pid|feedme_progress_start_elapsed|feedme_cookbook_stage|feedme_cookbook_pid|feedme_cookbook_uid|feedme_cookbook_start_elapsed)=(.*)$/);
...
function verifyNative(output, declared, notice = null, ordered = false) {
  ...
  if (notice) typeof notice==='function'?notice(notes[0]):verifyNotice(notes[0],notice);
...
function interrupted(output, beforeStop = false, identity = stageA, notice = verifyNotice) {
  const events=instrumentEvents(output), starts=events.filter(e=>e.code!==2), notices=events.filter(e=>e.code===2), a=starts[0];
  need(starts.length===1 && a.code===1 && `${a.class}#${a.test}`===identity && a.id==='AndroidJUnitRunner' && a.current==='1' && a.numtests==='1','Interrupted stage must be start-only');
  ...
  if(beforeStop) need(!/^INSTRUMENTATION_CODE:|INSTRUMENTATION_FAILED|Process crashed/m.test(output),'Stage terminated before witnessed stop');
    ...
    const terminal=[...output.matchAll(/^INSTRUMENTATION_CODE: (-?\d+)\s*$/gm)];
    need(terminal.length===1&&terminal[0][1]==='0'&&
      [...output.matchAll(/^INSTRUMENTATION_RESULT: shortMsg=Process crashed\.\s*$/gm)].length===1&&
      !/^INSTRUMENTATION_FAILED/m.test(output),'Witnessed stop must have one exact non-success crash terminal');
  }
  return notice(notices[0],'RETAINED_UNCLOSED');
}
...
const canonicalLong = (value, positive = false) => typeof value==='string' && (positive?/^[1-9][0-9]*$/:/^(0|[1-9][0-9]*)$/).test(value) &&
  value.length<=19 && BigInt(value)<=9223372036854775807n;
function verifyCookbookNotice(e, expected) {
  need(e?.code===2&&Object.keys(e).sort().join(',')==='code,feedme_cookbook_pid,feedme_cookbook_stage,feedme_cookbook_start_elapsed,feedme_cookbook_uid','Malformed cookbook notice');
  need(e.feedme_cookbook_stage===expected&&/^[1-9][0-9]*$/.test(e.feedme_cookbook_pid)&&
    /^[1-9][0-9]*$/.test(e.feedme_cookbook_uid)&&Number(e.feedme_cookbook_pid)<=2147483647&&Number(e.feedme_cookbook_uid)<=2147483647&&
    canonicalLong(e.feedme_cookbook_start_elapsed),'Invalid cookbook process/UID notice');
  return {pid:Number(e.feedme_cookbook_pid),uid:Number(e.feedme_cookbook_uid),startElapsedRealtime:e.feedme_cookbook_start_elapsed};
}
function cookbookMarker(raw, stage) {
  need(Buffer.isBuffer(raw)&&raw.length>0&&raw.length<=512&&['retained-unclosed','restored-and-reset'].includes(stage),'Cookbook marker bound/stage');
  const text=raw.toString('utf8'),m=JSON.parse(text),uuid=/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;
  const revision=text.match(/,"savedLocalRevision":([1-9][0-9]*),/)?.[1],elapsed=text.match(/,"startElapsedRealtime":(0|[1-9][0-9]*)\}$/)?.[1];
  need(m.version===1&&m.stage===stage&&uuid.test(m.planId)&&uuid.test(m.savedRecipeId)&&/^[0-9a-f]{64}$/.test(m.savedContentSha256)&&
    canonicalLong(revision,true)&&canonicalLong(elapsed)&&Number.isInteger(m.uid)&&m.uid>0&&m.uid<=2147483647&&
    Number.isInteger(m.pid)&&m.pid>0&&m.pid<=2147483647,'Invalid cookbook marker');
  const canonical=`{"version":1,"stage":${JSON.stringify(stage)},"planId":${JSON.stringify(m.planId)},"savedRecipeId":${JSON.stringify(m.savedRecipeId)},"savedContentSha256":${JSON.stringify(m.savedContentSha256)},"savedLocalRevision":${revision},"uid":${m.uid},"pid":${m.pid},"startElapsedRealtime":${elapsed}}`;
  need(raw.equals(Buffer.from(canonical)),'Noncanonical/extra cookbook marker fields');
  return {...m,savedLocalRevision:revision,startElapsedRealtime:elapsed};
}
function verifyCookbookLineage(old, restored, notice, uid) {
  for(const key of ['planId','savedRecipeId','savedContentSha256','savedLocalRevision','uid'])need(restored[key]===old[key],'Cookbook lineage changed');
  need(old.uid===uid&&restored.uid===notice.uid&&restored.pid!==old.pid&&
    BigInt(restored.startElapsedRealtime)>BigInt(old.startElapsedRealtime)&&
    restored.pid===notice.pid&&restored.startElapsedRealtime===notice.startElapsedRealtime,'Cookbook recovery process/UID mismatch');
}
...
function verifyTestDevice(serial, androidHome) {
  need(/^emulator-\d+$/.test(serial || '') && !['emulator-5554', 'emulator-5558'].includes(serial) &&
    typeof androidHome === 'string' && androidHome.trim().length > 0,
    'Explicit owned emulator other than user previews5554/5558 and ANDROID_HOME required');
}

async function main() {
  const serial=process.env.FEEDME_TEST_DEVICE;
  verifyTestDevice(serial, process.env.ANDROID_HOME);
  const adb=path.join(process.env.ANDROID_HOME,'platform-tools/adb'), began=new Date().toISOString();
  const base=path.join(root,'docs/verification/recalled-copy-drafts'), attempt=path.join(base,'attempts',began.replaceAll(':','-'));
  ...
  const report={startedAt:began,passed:false,serial,attemptDirectory:path.relative(root,attempt),commands:[],tests:[],native:[],artifacts:[],androidLint:[],screenshots:[],
    scope:'Focused tenth-batch source-bound checkpoint: complete current shared/progress/server JVM and PostgreSQL regressions;203 current-source Node checks plus10 byte-bound frozen-ninth historical parser regressions (not213 current-source checks); eight libraries and isolated progress app; actual foreground timer/cookbook controls and explicit synthetic progress identity/catalog/service. Native recalled-copy removal uses exact integrity-verified local redacted evidence after a synthetic service withdrawal, explicit original DELETE consent, retained queue/archive acknowledgements and tombstone fences; this is not live moderation or a cloud-only metadata removal API. Configured owner-only PostDraft HTTP/store tests cover five operations, exact lifecycle/authority/receipts/media cleanup using explicit synthetic fixtures; no post publication, live identity/content provider or implicit media grant. Existing configured media processing remains real bounded JPEG/PNG child decoding plus PostgreSQL jobs/leases/intents/cleanup exercised with synthetic object and malware/moderation adapters, not a deployed worker or accepting safety default. Native scope is25 timer +6 timer-only cancellation +6 progress host +4 actual timer host +5 cookbook host +2 fresh-process recoveries, totaling48 passed tests and2 separately witnessed interrupted starts with13 source-bound PNGs. Historical354 native tests and legacy VFS/process matrices are NOT rerun. Timer-only cancellation executes first in a fresh session-test UID and refuses WorkManager initialization. No real login/provider, deployment, complete F19/Make Again/social copy, signed offline manifest, physical power-loss, background timer/notification delivery, iOS or store-release acceptance. Only the explicitly supplied owned emulator is addressed; user previews5554/5558 are rejected.'};
  ...
  try {
    need(descriptor('scripts/verify-cookbook-processor.mjs').sha256==='6659895bbe6aa3996d44eefb8b917d5e2df92c82b366bc78a7f09ad5d9d7c9e4','Historical ninth runner changed');
    need(descriptor(ninthParserTest).sha256==='efd35842080e1d2259115baf189427ab326c497575db76a5d296ac4fa8b7366a','Historical ninth parser tests changed');
    need(descriptor('scripts/verify-cookbook-timer-media.mjs').sha256==='44454f2ba7c6d739ee7bf3604f0edf4a3dd23177543bfa79e4a29321d4056396','Historical eighth runner changed');
    ...
    const historical=historicalNinthClosure();
    const historicalReplay={
      scope:'Ten immutable ninth parser regressions against the accepted ninth retained source/dependency closure; historical regression replay, NOT ten current-source tests or a new ninth native acceptance.',
      baselineReceipt:retain(historical.receipt.path,'historical-node/ninth-receipt.json'),
      sourceManifestSha256:historical.sourceManifestSha256,
      files:historical.files.map(pair=>{
        const sandbox=write(`historical-node/ninth-workspace/${pair.source.path}`,read(pair.retained.path));
        need(sandbox.bytes===pair.source.bytes&&sandbox.sha256===pair.source.sha256,'Historical replay copy changed');
        return {originalSource:{originalPath:pair.source.path,bytes:pair.source.bytes,sha256:pair.source.sha256},acceptedRetainedSource:pair.retained,sandbox};
      }),
    };
    const historicalTest=historicalReplay.files.find(pair=>pair.originalSource.originalPath===ninthParserTest).sandbox.path;
    const historicalNodeIds=nodeDeclarations([historicalTest],fixed.historicalNode);
    report.historicalNodeReplay=historicalReplay;
    ...
    const cookbookHostIds=progressIds.filter(id=>id.startsWith(cookbookHostClass+'#'));
    const processIds=progressIds.filter(id=>!hostIds.includes(id)&&!timerHostIds.includes(id)&&!cookbookHostIds.includes(id));
    need(timerIds.length===fixed.timers&&timerOnlyIds.length===fixed.timerOnly&&hostIds.length===fixed.host&&timerHostIds.length===fixed.timerHost&&
      cookbookHostIds.length===fixed.cookbookHost&&processIds.length===fixed.processDeclarations&&JSON.stringify(processIds)===JSON.stringify([stageA,stageB,cookbookStageA,cookbookStageB].sort()),'Focused native declarations changed');
    ...
    report.cookbookScreenshotSources=verifyScreenshotSources(read('apps/android/src/androidTestProgress/kotlin/com/feedme/development/progress/AndroidProgressCookbookHostTest.kt').toString('utf8'),cookbookHostClass,cookbookPngMethods);
    const nodeFiles=walk(path.join(root,'scripts')).filter(f=>f.endsWith('.test.mjs')&&f!==ninthParserTest), nodeIds=nodeDeclarations(nodeFiles,fixed.currentNode);
    need(nodeIds.length+historicalNodeIds.length===fixed.node&&new Set([...nodeIds,...historicalNodeIds]).size===fixed.node,'Current/historical Node identities must be separate and exact');
    ...
    const currentTap=await run('node-tests-current-source',process.execPath,['--test','--test-reporter=tap',...nodeFiles]);
    const currentNode=verifyNodeTap(currentTap,nodeIds);
    const historicalTap=await run('node-tests-historical-ninth',process.execPath,['--test','--test-reporter=tap',historicalTest]);
    const historicalNode=verifyNodeTap(historicalTap,historicalNodeIds);
    // Verify the exact copied module/source closure again after it was executed/read. A matching
    // historical test total is not evidence for the current fifth cookbook source method.
    for(const pair of historicalReplay.files){const after=descriptor(pair.sandbox.path);need(after.bytes===pair.sandbox.bytes&&after.sha256===pair.sandbox.sha256,'Historical replay dependency changed');}
    historicalNinthClosure();
    report.node={tests:fixed.node,currentSource:{...currentNode,files:nodeFiles},
      historicalNinth:{...historicalNode,files:[historicalTest],provenance:historicalReplay},
      scope:'203 current-source Node checks plus10 byte-bound frozen-ninth historical parser regressions; not213 current-source checks.'};
    ...
    const cookbookHostLog=await device('progress-cookbook-host-tests',['shell','am','instrument','-w','-r','-e','class',cookbookHostClass,progressRunner],420000);
    report.native.push({name:'progress-cookbook-host',...verifyNative(cookbookHostLog,cookbookHostIds,null,true)});
    const cookbookCaptures=(await device('progress-cookbook-captures',['shell','run-as',progressPackage,'ls','-1','cache/progress-cookbook-ui-evidence'])).trim().split(/\r?\n/).sort();
    need(JSON.stringify(cookbookCaptures)===JSON.stringify(cookbookPngMethods.map(p=>p.name+'.png').sort()),'Cookbook capture inventory mismatch');
    for(const {name,identity}of cookbookPngMethods){const raw=await device('capture-'+name,['exec-out','run-as',progressPackage,'cat',`cache/progress-cookbook-ui-evidence/${name}.png`],10000,[0],true);
      report.screenshots.push({name,identity,...cookingPngInfo(raw),image:write(`cookbook-screenshots/${name}.png`,raw)});}
    need(report.screenshots.length===13&&new Set(report.screenshots.map(p=>p.name)).size===13,'Thirteen exact source-bound screenshots required');
    // Fifteen successful host cases explicitly closed their known owners. Stop only this freshly
    ...
    // Stage B above closed/reset the known owner. The separate cookbook checkpoint must start
    // in another process, and never consumes the timer checkpoint as cookbook authority.
    await device('before-cookbook-stage-a-stop',['shell','am','force-stop',progressPackage]);
    need(!(await pids('before-cookbook-stage-a-pids')).length,'Progress process did not stop before cookbook stage A');
    const cookbookChild=spawn(adb,['-s',serial,'shell','am','instrument','-w','-r','-e','class',cookbookStageA,progressRunner],{stdio:['ignore','pipe','pipe']});
    active={child:cookbookChild,output:'',terminal:false,name:'cookbook'};const cookbookRunning=active;
    for(const stream of [cookbookChild.stdout,cookbookChild.stderr])stream.on('data',b=>{cookbookRunning.output+=b.toString('utf8');if(cookbookRunning.output.length>4*1024*1024)cookbookChild.kill('SIGTERM');});
    cookbookChild.on('error',e=>{cookbookRunning.error=e.code;cookbookRunning.terminal=true;});
    cookbookChild.on('close',(code,signal)=>{cookbookRunning.exitCode=code;cookbookRunning.signal=signal;cookbookRunning.terminal=true;});
    const cookbookDeadline=performance.now()+60000;let cookbookNoticed=null;
    while(performance.now()<cookbookDeadline&&!cookbookRunning.terminal){
      if(/INSTRUMENTATION_STATUS: feedme_cookbook_stage=RETAINED_UNCLOSED\r?\n/.test(cookbookRunning.output)&&/INSTRUMENTATION_STATUS_CODE: 2\r?\n/.test(cookbookRunning.output)){
        cookbookNoticed=interrupted(cookbookRunning.output,true,cookbookStageA,verifyCookbookNotice);break;}await delay(150);}
    need(cookbookNoticed&&!cookbookRunning.terminal,'No live acknowledged cookbook checkpoint before deadline');
    const cookbookRaw=await device('cookbook-retained-marker',['exec-out','run-as',progressPackage,'cat','cache/progress-cookbook-process-evidence/retained.json'],10000,[0],true);
    const cookbookOld=cookbookMarker(cookbookRaw,'retained-unclosed');
    need(cookbookOld.uid===Number(uid)&&cookbookOld.uid===cookbookNoticed.uid&&cookbookOld.pid===cookbookNoticed.pid&&
      cookbookOld.startElapsedRealtime===cookbookNoticed.startElapsedRealtime&&JSON.stringify(await pids('live-cookbook-stage-a-pids'))===JSON.stringify([cookbookOld.pid]),'Cookbook checkpoint/live process mismatch');
    const cookbookProc=await device('live-cookbook-stage-a-status',['shell','run-as',progressPackage,'cat',`/proc/${cookbookOld.pid}/status`]);
    need(new RegExp(`^Uid:\\s+${uid}\\s+${uid}\\s+${uid}\\s+${uid}\\s*$`,'m').test(cookbookProc),'Cookbook checkpoint process not owned UID');
    write('cookbook-stage-a-before-stop.log',cookbookRunning.output);interrupted(cookbookRunning.output,true,cookbookStageA,verifyCookbookNotice);
    need(!cookbookRunning.terminal,'Cookbook stage ended before force-stop');
    await device('witnessed-cookbook-stage-a-stop',['shell','am','force-stop',progressPackage]);for(let i=0;i<40&&!cookbookRunning.terminal;i++)await delay(100);
    need(cookbookRunning.terminal&&!cookbookRunning.error,'Cookbook interrupted instrumentation did not terminate');
    write('cookbook-stage-a-interrupted.log',cookbookRunning.output);interrupted(cookbookRunning.output,false,cookbookStageA,verifyCookbookNotice);
    need(!(await pids('after-cookbook-stage-a-pids')).length,'Cookbook interrupted PID still live');active=null;
    report.cookbookWitnessedInterruption={identity:cookbookStageA,passedTest:false,checkpoint:cookbookOld,exitCode:cookbookRunning.exitCode,signal:cookbookRunning.signal,processAbsentAfterStop:true};
    const cookbookRecovery=await device('progress-cookbook-process-recovery',['shell','am','instrument','-w','-r','-e','class',cookbookStageB,progressRunner],180000);
    report.native.push({name:'progress-cookbook-process-recovery',...verifyNative(cookbookRecovery,[cookbookStageB],e=>verifyCookbookNotice(e,'RESTORED_AND_RESET'))});
    const cookbookRestored=cookbookMarker(await device('cookbook-restored-marker',['exec-out','run-as',progressPackage,'cat','cache/progress-cookbook-process-evidence/restored-and-reset.json'],10000,[0],true),'restored-and-reset');
    verifyCookbookLineage(cookbookOld,cookbookRestored,verifyCookbookNotice(instrumentEvents(cookbookRecovery).find(e=>e.code===2),'RESTORED_AND_RESET'),Number(uid));
    const cookbookMarkers=(await device('cookbook-process-marker-inventory',['shell','run-as',progressPackage,'ls','-1','cache/progress-cookbook-process-evidence'])).trim().split(/\r?\n/).sort();
    need(JSON.stringify(cookbookMarkers)===JSON.stringify(['restored-and-reset.json','retained.json']),'Cookbook process evidence inventory changed');
    const cookbookAgain=await device('cookbook-retained-marker-after-recovery',['exec-out','run-as',progressPackage,'cat','cache/progress-cookbook-process-evidence/retained.json'],10000,[0],true);
    need(cookbookAgain.equals(cookbookRaw),'Original cookbook interruption marker changed during recovery');
    report.cookbookProcessRecovery={restored:cookbookRestored,originalMarkerSha256:hash(cookbookRaw),originalMarkerUnchanged:true,
      scope:'Fresh process restores exact acknowledged local SavedRecipe body hash/revision, Plan and existing synthetic service bytes. Local restore is historical, not a new Save/receipt acknowledgement/download; absent finalization proof rejects Retry. Explicit reset proves terminal retained SIGNED_OUT and closed/redacted owner. Emulated acknowledged-commit process restart only, not interrupted transaction/power-loss/provider copy permission.'};
    report.witnessedInterruptions=[report.witnessedInterruption,report.cookbookWitnessedInterruption];
    report.nativeTests={tests:report.native.reduce((n,s)=>n+s.tests,0),failures:0,errors:0,skipped:0,witnessedInterruptions:2};
    need(report.native.length===7&&report.nativeTests.tests===48,'Only48 focused native passes in seven successful invocations allowed');
    const actualNative=report.native.flatMap(s=>s.identities),allDeclared=[...timerIds,...timerOnlyIds,...progressIds].sort();
    need(new Set(actualNative).size===48&&JSON.stringify([...actualNative,stageA,cookbookStageA].sort())===JSON.stringify(allDeclared),'Exact global native pass/interruption inventory mismatch');
    ...
    for(const pair of historicalReplay.files){const after=descriptor(pair.sandbox.path);need(after.bytes===pair.sandbox.bytes&&after.sha256===pair.sandbox.sha256,'Historical replay dependency changed before completion');}
    report.passed=true;
  } catch(error) { report.failure=error.message;process.exitCode=1; }
  finally {
    if(active){write(active.name==='cookbook'?'cookbook-stage-a-failed.log':'stage-a-failed.log',active.output);if(installedProgress)try{await device('failed-owned-stage-stop',['shell','am','force-stop',progressPackage]);}catch{}if(!active.terminal)active.child.kill('SIGTERM');}
    ...
    console.log(JSON.stringify({passed:report.passed,attemptDirectory:report.attemptDirectory,kotlin:report.kotlinTests,node:report.node?{total:report.node.tests,currentSource:report.node.currentSource.tests,historicalNinth:report.node.historicalNinth.tests}:null,native:report.nativeTests,sourceCount:report.sourceFiles?.length,artifacts:report.artifacts.length,failure:report.failure,sourceFailure:report.sourceFailure}));
  }
}
export {verifyTestDevice,historicalNinthClosure,verifyNodeTap,cookbookMarker,verifyCookbookNotice,verifyCookbookLineage,cookbookHostClass,cookbookStageA,cookbookStageB,cookbookPngMethods,sourceFiles,declarations,nodeDeclarations,instrumentEvents,verifyNative,verifyNotice,interrupted,marker,verifyProgressManifest,verifyProgressScreenshots,verifyScreenshotSources,verifyInstalledManifest};
