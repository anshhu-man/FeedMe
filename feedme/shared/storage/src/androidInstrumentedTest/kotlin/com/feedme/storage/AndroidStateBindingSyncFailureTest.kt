 ...
 * This does not establish public recovery-factory journal support or completed session access.
 */
@RunWith(AndroidJUnit4::class)
class AndroidStateBindingSyncFailureTest {
    ...
    @Test fun journalSyncFailureCannotAcknowledgeInitialOrReplayedPrivateBinding() = runBlocking(dispatcher) {
        bindingAndReplay(SyncPoint.JOURNAL)
    }

    @Test fun databaseSyncFailureCannotAcknowledgeInitialOrReplayedPrivateBinding() = runBlocking(dispatcher) {
        bindingAndReplay(SyncPoint.DATABASE)
    }

    @Test fun directorySyncFailureCannotPromoteVisibleBindingOrReplayToAcknowledgement() = runBlocking(dispatcher) {
    ...
    private suspend fun bindingAndReplay(point: SyncPoint) {
        ...
        val target = f.database.inspectPlannedBinding(OWNER, f.plan).valueOrFail().target!!.copyForStorage()
        inject(f, point, "bind-${point.label}")
        f = reopen(f)
        val initial = f.database.inspectPlannedBinding(OWNER, f.plan).valueOrFail()
        if (point == SyncPoint.DIRECTORY) assertNotNull(initial.record)
        initial.record?.let { assertArrayEquals(BODY.copyForCodec(), it.payload.copyForCodec()) }
        val receipt = f.database.bindPlannedActivation(OWNER, f.plan, 2, BODY).valueOrFail()
        assertEquals((initial.record?.revision ?: 0) + 1, receipt.record!!.revision)
        assertArrayEquals(target, receipt.target!!.copyForStorage())
        assertArrayEquals(BODY.copyForCodec(), receipt.record!!.payload.copyForCodec())
        assertEquals(aliases, aliases(f.box)); assertNoKeyEffects(f)

        inject(f, point, "replay-${point.label}")
        f = reopen(f)
        val selected = f.database.inspectPlannedBinding(OWNER, f.plan).valueOrFail()
        assertNotNull(selected.record)
        assertArrayEquals(BODY.copyForCodec(), selected.record!!.payload.copyForCodec())
        val replay = f.database.bindPlannedActivation(OWNER, f.plan, 2, BODY).valueOrFail()
        assertEquals(selected.record!!.revision + 1, replay.record!!.revision)
        assertArrayEquals(target, replay.target!!.copyForStorage())
        assertArrayEquals(BODY.copyForCodec(), replay.record!!.payload.copyForCodec())
        assertArrayEquals(originalPlan, f.plan.copyForStorage())
        assertEquals(aliases, aliases(f.box)); assertNoKeyEffects(f)
        // No resumeBound call, scoped handle, lease, work seal or control completion in this fixture.
    }
    ...
    private suspend fun inject(f: Fixture, point: SyncPoint, label: String) {
        ...
        val outcome: PortResult<StateRecordInspection>
        val stats: LongArray
        try {
            outcome = f.database.bindPlannedActivation(OWNER, f.plan, 2, BODY)
            stats = SqliteSyncFailureInjector.statistics()
        } finally { SqliteSyncFailureInjector.restore() }
        assertTrue("Failed native synchronization cannot return a binding receipt", outcome is PortResult.Failure)
        ...
        InstrumentationRegistry.getInstrumentation().addResults(Bundle().apply {
            putString("data_binding_sync_$label",
                "sqlite=${point.sqlite};failures=1;receipts=0;vfs=1;delegatedUnlink=$unlinked")
        })
    }
    ...
    private class NativeTrace(private val native: AndroidStateVault) : PlannedStateVault by native {
        var allocated = 0; var created = 0; var deleted = 0
        ...
        override fun deleteOwnerKey(keyId: String) { deleted++; native.deleteOwnerKey(keyId) }
    }
    ...
    companion object {
        private val OWNER = StorageScope("native-binding-sync-fixture", ActorKind.ACCOUNT, "private-binding-owner")
