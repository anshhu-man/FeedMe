...
fun interface KitchenIngredientSearch {
    ...
    /** Explicit targeted-read support; a search-only adapter must never list/fallback. */
...
/** Independent domain admission, also used by non-HTTP callers. UUID identity is case-free. */
internal fun checkedIngredientIds(ids: List<UUID>): List<UUID> {
    if (ids.size !in 1..50) throw KitchenFailure(KitchenFailureCode.INPUT_INVALID)
    val copy = ids.toList()
    if (copy.toSet().size != copy.size) throw KitchenFailure(KitchenFailureCode.INPUT_INVALID)
    return copy
}
...
enum class KitchenFailureCode(val status: Int) {
    INPUT_INVALID(422), UNAUTHENTICATED(401), POLICY_BLOCKED(403), PREFERENCES_UNAVAILABLE(404), PANTRY_ITEM_UNAVAILABLE(404),
