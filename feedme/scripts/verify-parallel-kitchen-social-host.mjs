...
const parallelCounts = {sync: 108, planning: 40, mealflow: 129, app: 57, session: 593, server: 110, postgresql: 145, startupNative: 12};
...
const destination = path.join(root, 'docs/verification/parallel-kitchen-social-host');
...
const report = {
  ...
  scope: "Parallel V1 kitchen editing/social HTTP/retained native-host component batch. Actual retained-session meal form and presentation host, authenticated bounded ingredient catalog lookup/cache, explicit pantry reports and exact durable meal commands remain separate from cooking/save/share authority. Six native Compose presentation tests retain synthetic view-state fixtures. Nine additional tests exercise the actual retained meal host and public native session/encrypted stores, using explicitly synthetic account verification and transport. These cover Activity recreation, system Back, exact native drafts and original-key retries, synchronous invalidation and late reply fencing, not a real provider/HTTP/cooking journey or an actual delayed native Save acknowledgement. Pantry/preferences reuse the existing durable queue with exact version/body admission, explicit dispatch and pending-preference planning guards. Attempted permanent conflicts remain blocked for a later verified resolution API. Four planning plus fourteen circle/invitation canonical HTTP operations are explicitly opt-in to actual PostgreSQL PlansStore/CirclesStore and mandatory trusted verifier/current identity/catalog/block adapters; default Main remains health-degraded and product-503. HTTP tests use real Ktor/CIO and PostgreSQL with explicitly synthetic identity/catalog fixtures, exact numeric/body validation and same-command outcome reconciliation; no production provider or deployment exists. Seven separate host-witnessed forced terminations occur at acknowledged existing-startup/abort/close boundaries; only the seven fresh-process recovery methods are passed tests, never the deliberately killed starts. Original authenticated plans, confirmation, lifecycle and close ownership remain enforced. Existing native ownership, 39 injected-VFS labels and five controlled process stages remain; test helper packaging is checked for four ABIs, runtime device is API35 ARM64 only. These are not transaction-crash, OS-fsync failure, physical-power-loss, general orphan/hot-journal, real-provider/UI-consent, API26/iOS, physical-device, signing or release acceptance. Approved V1 remains44 P1 features with10 deferred; historical demo bytes remain unchanged.",
...
try {
  ...
  if(report.kotlinTests.shared!==1787+(parallelCounts.sync-103)+(parallelCounts.mealflow-82)+(parallelCounts.app-34) || report.kotlinTests.tests!==report.kotlinTests.shared+parallelCounts.server+parallelCounts.postgresql)throw new Error('Unexpected exact Kotlin/server/PostgreSQL totals.');
  report.androidLint = [];
