 ...
 */
internal class NativeTimerTestSandbox private constructor(base: Context, private val directory: File) {
    ...
    companion object {
        const val DIRECTORY_PREFIX = "foreground-timer-native-"
        ...
        fun create(base: Context, stage: (NativeTimerFixtureStage) -> Unit): NativeTimerTestSandbox {
            stage(NativeTimerFixtureStage.SANDBOX_IDENTITY)
            require(base.packageName == "com.feedme.mealflow.test") { "Only the isolated application test UID is authorized" }
            ...
            var created = false
            try {
                stage(NativeTimerFixtureStage.SANDBOX_NAMESPACE)
                require(aliases().isEmpty()) { "Existing native namespaces are not cleanup authority" }
                stage(NativeTimerFixtureStage.SANDBOX_PARENT)
                ...
                stage(NativeTimerFixtureStage.SANDBOX_MKDIR)
                ...
                return NativeTimerTestSandbox(base, directory)
            } finally {
