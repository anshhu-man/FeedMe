-- LOCAL additive V051/V052 inventory integration; no grants, worker activation,
-- deployment, shared-history disposition, provider effect or full erasure completion.
-- All prior migration bytes stay unchanged. Exact-private reuse evidence/settings
-- use the existing accepted-job/backend-PID/transaction/lease capability. Catalog
-- relationship evidence and shared conversations are never deleted by this slice.
-- The original fixed-table NOWAIT write-quiescence release gate remains.
-- Correct the V051 owner discriminator: identity.principals uses kind='user';
-- actor_kind='account' is the separate private-domain namespace, not that column.
 ...
 RETURN EXISTS(
  ...
  WHERE s.backend_pid=pg_backend_pid() AND s.transaction_id=txid_current()
    ...
    AND ((p_relation IN ('profile.onboarding_decisions'::regclass,
           ...
           'social.account_privacy'::regclass)
...
CREATE OR REPLACE FUNCTION planning.guard_reuse_owner_insert() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER
 SET search_path=pg_catalog,pg_temp SET row_security=off AS $feedme_reuse_owner$
DECLARE account_id uuid;
BEGIN
 SELECT user_id INTO account_id FROM identity.principals WHERE environment=NEW.environment AND id=NEW.principal_id AND kind='user' AND guest_session_id IS NULL;
 ...
 PERFORM 1 FROM identity.principals WHERE environment=NEW.environment AND id=NEW.principal_id AND user_id=account_id AND kind='user' AND guest_session_id IS NULL AND status='active' FOR SHARE NOWAIT;
...
REVOKE ALL ON FUNCTION planning.guard_reuse_owner_insert() FROM PUBLIC;

CREATE OR REPLACE FUNCTION planning.guard_reuse_window() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,pg_temp SET row_security=off AS $feedme_reuse_window$
BEGIN
 IF TG_OP='DELETE' AND TG_RELID='planning.reuse_windows'::regclass AND OLD.actor_kind='account'
   AND identity.account_erasure_delete_allowed(TG_RELID,OLD.environment,NULL,OLD.principal_id) THEN RETURN OLD; END IF;
...
REVOKE ALL ON FUNCTION planning.guard_reuse_window() FROM PUBLIC;

CREATE FUNCTION planning.guard_reuse_account_erasure() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,pg_temp SET row_security=off
AS $feedme_reuse_erasure$
BEGIN
 IF TG_OP='DELETE' AND OLD.actor_kind='account'
   AND TG_RELID IN ('planning.reuse_requests'::regclass,'planning.reuse_pages'::regclass)
   AND identity.account_erasure_delete_allowed(TG_RELID,OLD.environment,NULL,OLD.principal_id) THEN RETURN OLD; END IF;
 RAISE EXCEPTION 'Reuse evidence is immutable' USING ERRCODE='23514';
END;
$feedme_reuse_erasure$;
REVOKE ALL ON FUNCTION planning.guard_reuse_account_erasure() FROM PUBLIC;
DROP TRIGGER reuse_evidence_immutable ON planning.reuse_requests;
CREATE TRIGGER reuse_evidence_immutable BEFORE UPDATE OR DELETE ON planning.reuse_requests
 FOR EACH ROW EXECUTE FUNCTION planning.guard_reuse_account_erasure();
DROP TRIGGER reuse_evidence_immutable ON planning.reuse_pages;
CREATE TRIGGER reuse_evidence_immutable BEFORE UPDATE OR DELETE ON planning.reuse_pages
 FOR EACH ROW EXECUTE FUNCTION planning.guard_reuse_account_erasure();

CREATE FUNCTION social.guard_account_privacy_erasure() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,pg_temp SET row_security=off
AS $feedme_privacy_erasure$
BEGIN
 IF TG_OP='DELETE' THEN
  IF TG_RELID='social.account_privacy'::regclass
    AND identity.account_erasure_delete_allowed(TG_RELID,OLD.environment,OLD.user_id,NULL) THEN RETURN OLD; END IF;
 END IF;
 RAISE EXCEPTION 'Privacy erasure requires accepted account job' USING ERRCODE='23514';
END;
$feedme_privacy_erasure$;
REVOKE ALL ON FUNCTION social.guard_account_privacy_erasure() FROM PUBLIC;
CREATE TRIGGER account_privacy_erasure BEFORE DELETE ON social.account_privacy
 FOR EACH ROW EXECUTE FUNCTION social.guard_account_privacy_erasure();
CREATE TRIGGER account_privacy_no_truncate BEFORE TRUNCATE ON social.account_privacy
 FOR EACH STATEMENT EXECUTE FUNCTION social.guard_account_privacy_erasure();

-- UPDATE of private immutable reuse evidence and every original catalog/REUSE
-- TRUNCATE guard remain unconditional; reuse quota UPDATE keeps its exact rules.
...
DECLARE
 ...
 account_ops constant text[]:=ARRAY['bootstrapAccount','logoutSession','acceptAccountTerms','updateMe','requestAccountDeletion',
   'createPostDraft','updatePostDraft','deletePostDraft','updatePrivacySettings'];
 ...
 fenced_tables constant text[]:=ARRAY[
   ...
   'social.account_privacy','social.direct_threads','social.thread_messages','social.thread_read_watermarks',
 ...
 IF EXISTS(SELECT 1 FROM platform.idempotency i
   WHERE starts_with(i.principal_scope,p_environment||':account:') AND i.principal_scope<>account_scope
     AND ((i.operation_id='blockUser' AND i.response_json->>'targetUserId'=accepted.user_id::text)
       ...
       OR (i.operation_id='sendMessage' AND i.response_json#>>'{sender,userId}'=accepted.user_id::text))) THEN
  RETURN 'related_data';
 END IF;
