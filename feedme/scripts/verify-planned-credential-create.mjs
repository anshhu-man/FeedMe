...
// Credential CREATE plans only: authenticated intent, exact control barrier and existing-only abort.
// Requires existing JDK17, SDK36, local PostgreSQL binaries and a booted local emulator.
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const destination = path.join(root, 'docs/verification/planned-credential-create');
...
const report = {
  ...
  scope: 'Bounded authenticated credential CREATE plans persisted in independent encrypted control before native credential material writes; exact payload/incarnation/revision binding, immutable plan-qualified files, durable consumed-plan abort, and restricted existing-only inspect/abort recovery. Startup inspection never confirms deletion; explicit confirmation durably records abort before native recovery opens, and completion follows exact cleanup and durability acknowledgement. Credential plans resolve before new private-data/work composition, but private-data/work writes themselves remain unplanned. Fresh shared/server/isolated-PostgreSQL/Node regressions, five Android library builds/lint, and isolated real native storage, credential-plan, exact-cancellation and separately invoked runtime/control integration tests. Tests use fake or synthetic identity verification and explicit test orchestration, not a real provider or application UI. Only storage has two separate instrumentation-process stages; credential-plan tests cover controlled close/reopen and injected native boundaries, not process-hard-kill or power-loss proof. Credential support remains gated to API 27+ (API 26 NOT_CONFIGURED). Hardlink platform-denial and notification branches are retained explicitly; platform denial is not existing-hardlink guard proof and permission-unavailable is not delivered-notification cancellation proof. No refresh-plan recovery, arbitrary orphan cleanup, private-data/work runtime plan integration, full first-factory initialization recovery, production timer receiver/worker, app scheduling/recovery/confirmation wiring, provider sign-in, physical-device, iOS or release claim.',
...
function retainNativeEvidence(suite, notBefore) {
  ...
  if (suite.module === 'session') {
    ...
    if (android.apkMetadata?.path !== `${nativePath}/output-metadata.json` ||
      ...
      throw new Error('Session APK metadata changed since the native smoke captured it.');
    }
  }
