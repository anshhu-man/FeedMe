...
// Exact existing-only private-data activation recovery and explicit empty-only abort.
// Requires existing JDK17, SDK36, local PostgreSQL binaries and a booted local emulator.
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const destination = path.join(root, 'docs/verification/planned-state-recovery');
