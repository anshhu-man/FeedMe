-- Recognize account-owned Inbox notifications, read watermarks and read-command receipts.
-- Preserve exact related-data holds until shared-history erasure is actually implemented.
-- Only the accepted-job capability may remove its own read watermark. No shared
-- notification deletion, role grants, live migration or account cleanup is run here.
