...
// Verified-session composition components; verifier remains an explicit integration boundary.
// Requires existing JDK17, SDK36, local PostgreSQL binaries and a booted local emulator.
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const destination = path.join(root, 'docs/verification/session-composition');
...
const report = {
  ...
  scope: 'Private session runtime and exact activation-link codec; independently checked credential incarnation, work origin, current private-data target and trusted configuration digest, terminal close/retirement gates, scoped repository/transport handles and offline-private behavior. Runtime tests use a fake or synthetic verifier, not actual provider proof. Fresh shared/server/isolated-PostgreSQL/Node regressions, five Android library builds/lint, and isolated native storage, credential, cancellation and separate factory/runtime-integration tests. Native integration uses real stores and OS cancellation but not a UI or a real provider. Only storage includes two separate instrumentation-process stages; no process-hard-kill or power-loss proof. Android credential factory remains gated to API 27+ (API 26 NOT_CONFIGURED). Hardlink and notification branches are retained explicitly; platform denial is not existing-hardlink guard proof, and permission-unavailable is not delivered-notification cancellation proof. No production timer receiver/worker or app scheduling/recovery wiring, guest merge, account-deletion workflow, physical-device, iOS or release claim.',
