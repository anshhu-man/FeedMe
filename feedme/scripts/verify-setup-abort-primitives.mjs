...
// Source-bound exact setup abort primitives; not integrated confirmation or startup recovery.
...
const destination = path.join(root, 'docs/verification/setup-abort-primitives');
...
const report = {
  ...
  scope: 'M1.05d.5b.2b.3.6b exact private-binding and setup-work abort primitives only: authenticated exact plan consumption, preserved abort provenance, and a fresh changed acknowledgement on every initial or replayed abort. Private data cleanup accepts an authenticated exact plan with zero rows or the sole reserved schema2 binding matching the trusted callers expected bytes; it rejects foreign/newer selection and additional rows or tombstones. Binding removal and a changed consumed receipt share one transaction before exact key deletion; consumed-state replay requires the original native receipt and a fresh acknowledgement. Storage does not validate session JSON or infer consent: future trusted composite coordination must derive the canonical original session identities and independently acknowledge confirmation. Work abort accepts exact PREPARED, SELECTED, untouched SEALED or ABORTED state, never calls OS work/policies or allocates identity, and preserves an authenticated consumed marker so a later exact predecessor can be planned; selecting a successor fences the old plan. Planning alone does not reserve the predecessor. All prior already-open diagnostics, actual runtime late-publication, native metadata and component regressions remain. Twelve new initial/replay VFS labels extend the retained 27 to 39; all five earlier separate-process stages remain. Injected SQLite VFS errors are not OS errno failures or physical power loss and direct owned-connection fixtures do not prove public hot-journal recovery. Application response-loss wrappers are reported distinctly. This slice does not integrate explicit composite abort confirmation, cross-store ordering, restricted factory recovery, startup completion or the integrated process-separation matrix. Provider/bootstrap/configuration approval, first-factory/close/journal recovery, API26 native recovery, product UI/domain wiring, physical devices, full Xcode/iOS and release acceptance remain open. Fresh Kotlin/server/isolated PostgreSQL/Node regressions, five Android library builds/lint and test-only four-ABI JNI provenance are source-bound. The historical demo and public GitHub snapshot remain unchanged; no deployment or publication.',
...
try {
  ...
  if (!/^# tests 142$/m.test(nodeLog) || !/^# pass 142$/m.test(nodeLog) || !/^# fail 0$/m.test(nodeLog) || !/^# skipped 0$/m.test(nodeLog))
    throw new Error('Expected all 142 Node regressions without skips.');
  for (const [directory, name, expected] of [
    ...
    ['shared/storage/build/test-results/jvmTest', 'storage', 444],
    ['shared/sync/build/test-results/jvmTest', 'sync', 103],
    ['shared/kitchen/build/test-results/jvmTest', 'kitchen', 142],
    ['shared/session/build/test-results/jvmTest', 'session', 489],
  ...
  report.node = {tests: 142, failures: 0, errors: 0, skipped: 0};
  report.androidLint = [];
