...
const report = {
  ...
  scope: 'Isolated native credential/private-file and authenticated CREATE-plan tests, exact timer/worker cancellation and separate real native-store/OS integration, including private runtime, read-only diagnostics, confirmed readable-empty setup discard and independent-control planned-create abort with a synthetic verifier. CREATE recovery uses only an authenticated exact plan and an existing-only inspect/abort handle; explicit abort intent precedes native cleanup, with durable consumption before completion. Diagnostics inspect already-open stores only; they do not initialize factories, prove usable credentials or grant authority. Discard revalidates exact incomplete targets and empty native-work/private-record state. Controlled close/reopen and injected native boundaries are covered; notification and hardlink branches are reported explicitly. No refresh-plan, planned private-data/work, full first-factory initialization or generalized orphan recovery. Not actual provider sign-in, production timer receiver/worker or app scheduling/recovery wiring, FeedMe UI/auth/confirmation integration, process-hard-kill, physical-device, power-loss or iOS proof.',
...
try {
  ...
  instrument('integration', 'com.feedme.session.AndroidLocalRetirementIntegrationTest', 10);
