...
internal class KitchenHttpInput private constructor(
    ...
    suspend fun readBody(channel: ByteReadChannel, validator: ContractBodyValidator): JsonObject? {
        val buffer = ByteArray(if (hasBody) KitchenHttpConfiguration.MAX_REQUEST_BYTES + 1 else 1)
        var size = 0
        try {
            while (true) {
                ...
                size += read
                if (size == buffer.size) invalidKitchen()
            }
            currentCoroutineContext().ensureActive()
            if (contentLength != null && contentLength != size.toLong()) invalidKitchen()
            return body(buffer.copyOf(size), validator)
        } catch (cancelled: CancellationException) { throw cancelled }
        catch (_: IOException) { currentCoroutineContext().ensureActive(); invalidKitchen() }
    ...
    companion object {
        fun parse(operation: String, headers: Headers, query: Parameters, paths: Parameters): KitchenHttpInput {
            ...
            val allowedQuery = when (operation) {
                ...
                "searchIngredients" -> setOf("q", "cursor", "limit")
                else -> emptySet()
            }
            ...
            return KitchenHttpInput(operation, bearer, key, ingredient, ifMatch, cursor, search, limit, length, media)
        }
    ...
    try {
        ...
        val reply = runInterruptible(configuration.databaseDispatcher) {
            when (operation) {
                ...
                "searchIngredients" -> store.searchIngredients(actor, input.query, input.cursor, input.limit, configuration.ingredientSearch)
...
private fun kitchenReply(result: CommandResult): StoredReply = when (result) {
