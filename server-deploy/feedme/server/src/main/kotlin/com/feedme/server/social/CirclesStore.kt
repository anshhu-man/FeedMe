...
class CirclesStore private constructor(
    ...
    fun acceptInvitation(actor: VerifiedSocialAccount, key: UUID, body: JsonObject): CommandResult {
        ...
        val result = command(actor, "acceptInvitation", key, body = input, replay = { c, cached ->
            acceptanceCircle(c, actor, tokenHash, cached)
        }) { c ->
            ...
            if (invite.status == "exhausted" && invite.consumedBy == actor.accountId) {
                ...
                return@command acceptanceMemberReceipt(c, previous)
            }
            ...
            acceptanceMemberReceipt(c, joined)
        }
        // Preserve the established token-free Member receipt format, including legacy receipts.
        // The target is projected only after a fresh actual principal/relationship check; no list
        // lookup, caller target, parsed token identifier or newly read Member replaces the original.
        return when (result) {
            is CommandResult.Applied -> CommandResult.Applied(invitationAcceptance(actor, tokenHash, result.reply))
            is CommandResult.Replayed -> CommandResult.Replayed(invitationAcceptance(actor, tokenHash, result.reply))
            else -> result
        }
    }

    private fun acceptanceMemberReceipt(c: Connection, member: MemberRow): StoredReply {
        val original = memberJson(c, member)
        if (validator.validateSchema("Member", original.toString().encodeToByteArray(throwOnInvalidSequence = true)) != BodyValidationResult.Valid)
            fail(SocialFailureCode.STORAGE_UNAVAILABLE)
        return StoredReply(200, original, "\"${member.version}\"")
    }

    private fun acceptanceCircle(c: Connection, actor: VerifiedSocialAccount, tokenHash: String, original: StoredReply): UUID {
        if (original.status != 200) fail(SocialFailureCode.STORAGE_UNAVAILABLE)
        val candidate = findInvitation(c, tokenHash) ?: fail(SocialFailureCode.INVITATION_UNAVAILABLE)
        val current = circle(c, candidate.circleId); val invite = invitation(c, candidate.id)
        val currentMember = member(c, current.id, actor.accountId); requireActive(current, currentMember)
        if (invite.circleId != current.id || invite.tokenHash != tokenHash || invite.status != "exhausted" ||
            invite.consumedBy != actor.accountId || invite.consumedGeneration != currentMember!!.generation ||
            currentMember.id != uuid(original.body!!.jsonObject, "id")) fail(SocialFailureCode.INVITATION_UNAVAILABLE)
        joinPolicy(c, actor, current, invite)
        requireCurrentMemberReply(c, currentMember, original)
        return invite.circleId
    }

    private fun invitationAcceptance(actor: VerifiedSocialAccount, tokenHash: String, original: StoredReply): StoredReply = read(actor) { c ->
        val target = acceptanceCircle(c, actor, tokenHash, original)
        val document = JsonObject(original.body!!.jsonObject + ("circleId" to JsonPrimitive(target.toString())))
        val projected = reply("acceptInvitation", 200, document)
        StoredReply(projected.status, projected.body, original.etag)
    }
