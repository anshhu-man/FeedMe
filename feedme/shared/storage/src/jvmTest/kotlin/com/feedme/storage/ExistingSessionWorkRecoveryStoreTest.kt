...
@OptIn(WorkRecoveryCompositionApi::class, ExperimentalCoroutinesApi::class)
class ExistingSessionWorkRecoveryStoreTest {
    ...
    @Test fun beforeOpenCapabilitiesAreFencedWithoutLazyInitializationOrProofSigning() = runBlocking {
        fixture { f ->
            ...
            rejected(FailureReason.STALE_SESSION, owner.verifyOriginPlan(1, PROPOSAL, f.proof))
            rejected(FailureReason.STALE_SESSION, owner.verifyOriginPredecessor(f.original, PROPOSAL, f.proof))
    ...
    @Test fun openAndRepeatedReadAndProofVerificationPreserveExactBytesKeysAndRevision() = runBlocking {
        fixture { f ->
            f.prepare(); val owner = f.owner(); val before = Files.readAllBytes(f.file)
            val keys = f.vault.keys.toMap(); ok(owner.open())
            repeat(3) {
                same(f.original, record(owner))
                ok(owner.verifyOriginPlan(f.original.revision, PROPOSAL, f.proof))
                ok(owner.verifyOriginPredecessor(f.original, PROPOSAL, f.proof))
            }
    ...
    @Test fun samePayloadCasMakesOneChangedAcknowledgedRevisionAndNeverAllocatesOrDeletesKeys() = runBlocking {
        fixture { f ->
            ...
            assertEquals(1L, f.scalar("SELECT count(*) FROM feedme_records"))
            ok(owner.verifyOriginPlan(f.original.revision, PROPOSAL, f.proof))
            rejected(FailureReason.CONFLICT, owner.verifyOriginPredecessor(f.original, PROPOSAL, f.proof))
            rejected(FailureReason.INVALID_DATA, owner.verifyOriginPredecessor(written, PROPOSAL, f.proof))
        }
    }
    ...
    @Test fun beforeCommitFailureAndLostApplicationReceiptRemainFailureUntilFreshChangedCas() = runBlocking {
        for (fault in WorkCommitFault.entries) fixture { f ->
            f.prepare(); val owner = f.owner(); ok(owner.open()); f.sql.commitFault = fault
            rejected(if (fault == WorkCommitFault.BEFORE) FailureReason.STORAGE_FAILURE else FailureReason.OUTCOME_UNKNOWN,
                owner.compareAndSet(1, bytes("exact-retained-work-abort-marker")))
            val observed = record(owner)
            assertEquals(if (fault == WorkCommitFault.BEFORE) 1L else 2L, observed.revision)
            ...
            assertEquals(if (fault == WorkCommitFault.BEFORE) 1 else 2, f.sql.writeCommits)
        }
    }
    ...
    @Test fun cancelledCallerAfterAcknowledgedCommitGetsNoSuccessAndCanObserveExactWrittenRecord() = runTest {
        fixture { f ->
            ...
            val writing = async(start = CoroutineStart.LAZY) { owner.compareAndSet(1, bytes("cancelled-return-work-marker")) }
    ...
    @Test fun proofRevisionProposalAndBothMacHalvesRemainBoundWithoutMutation() = runBlocking {
        fixture { f ->
            f.prepare(); val owner = f.owner(); ok(owner.open()); f.observe(); val before = Files.readAllBytes(f.file)
            rejected(FailureReason.INVALID_DATA, owner.verifyOriginPlan(2, PROPOSAL, f.proof))
            rejected(FailureReason.INVALID_DATA, owner.verifyOriginPlan(1, bytes("foreign-proposal"), f.proof))
            for (index in listOf(0, 15, 31, 32, 47, 63)) {
                val changed = f.proof.copyForCodec(); changed[index] = (changed[index].toInt() xor 1).toByte()
                rejected(FailureReason.INVALID_DATA, owner.verifyOriginPlan(1, PROPOSAL, PrivateBytes(changed)))
            }
            ok(owner.verifyOriginPredecessor(f.original, PROPOSAL, f.proof)); f.assertReadOnly(before)
        }
    }

    @Test fun malformedProofInputsAreRejectedAndNeverMintReplacementProofs() = runBlocking {
        fixture { f ->
            f.prepare(); val owner = f.owner(); ok(owner.open()); f.observe(); val before = Files.readAllBytes(f.file)
            for (revision in listOf(-1L, 0L, Long.MAX_VALUE - 1, Long.MAX_VALUE))
                rejected(FailureReason.INVALID_DATA, owner.verifyOriginPlan(revision, PROPOSAL, f.proof))
            for (size in listOf(0, 4097)) rejected(FailureReason.INVALID_DATA,
                owner.verifyOriginPlan(1, PrivateBytes(ByteArray(size)), f.proof))
            for (size in listOf(0, 63, 65)) rejected(FailureReason.INVALID_DATA,
                owner.verifyOriginPredecessor(f.original, PROPOSAL, PrivateBytes(ByteArray(size))))
            for (size in listOf(0, 32_769)) rejected(FailureReason.INVALID_DATA,
                owner.verifyOriginPredecessor(SessionControlRecord(1, PrivateBytes(ByteArray(size))), PROPOSAL, f.proof))
            assertTrue(f.sql.prepared.isEmpty()); assertEquals(0, f.vault.indexes); f.assertReadOnly(before)
        }
    }

    @Test fun originalPredecessorProofBindsExactRawWhitespaceNotOnlyDecodedStateOrRevision() = runBlocking {
        fixture { f ->
            f.prepare(bytes(" { \"version\" : 1, \"state\" : \"idle\" } \n"))
            val owner = f.owner(); ok(owner.open()); f.observe(); val before = Files.readAllBytes(f.file)
            rejected(FailureReason.CONFLICT, owner.verifyOriginPredecessor(SessionControlRecord(1, IDLE), PROPOSAL, f.proof))
            ok(owner.verifyOriginPredecessor(f.original, PROPOSAL, f.proof)); f.assertReadOnly(before)
            f.rewriteRecord(IDLE, 1); f.observe(); val changed = Files.readAllBytes(f.file)
            ok(owner.verifyOriginPlan(1, PROPOSAL, f.proof))
            rejected(FailureReason.INVALID_DATA, owner.verifyOriginPredecessor(record(owner), PROPOSAL, f.proof))
            rejected(FailureReason.CONFLICT, owner.verifyOriginPredecessor(f.original, PROPOSAL, f.proof))
            f.assertReadOnly(changed)
        }
    }

    @Test fun equalRevisionForeignInstallCannotAuthenticateAnotherStoresPlan() = runBlocking {
        fixture { first -> fixture { second ->
            first.prepare(); second.prepare(); val owner = second.owner(); ok(owner.open())
            assertEquals(first.original.revision, second.original.revision); same(first.original.payload, second.original.payload)
            second.observe(); val before = Files.readAllBytes(second.file)
            rejected(FailureReason.INVALID_DATA, owner.verifyOriginPlan(1, PROPOSAL, first.proof))
            rejected(FailureReason.INVALID_DATA, owner.verifyOriginPredecessor(second.original, PROPOSAL, first.proof))
            second.assertReadOnly(before)
        } }
    }

    @Test fun sameInstallNewOwnerIncarnationCannotVerifyOldProofAfterExplicitReopen() = runBlocking {
        fixture { f ->
            f.prepare(); val oldProof = f.proof; f.replaceOwner()
            val owner = f.owner(); ok(owner.open()); f.observe(); val before = Files.readAllBytes(f.file)
            same(IDLE, record(owner).payload)
            rejected(FailureReason.INVALID_DATA, owner.verifyOriginPlan(1, PROPOSAL, oldProof))
            rejected(FailureReason.INVALID_DATA, owner.verifyOriginPredecessor(record(owner), PROPOSAL, oldProof))
            f.assertReadOnly(before)
        }
    }
    ...
    @Test fun everyReadyReadCasAndProofRevalidatesStrictShapeBeforeAnyWrite() = runBlocking {
        for (damage in listOf("extra-row", "garbage", "receipt", "ciphertext", "missing")) fixture { f ->
            ...
            rejected(FailureReason.STORAGE_FAILURE, owner.verifyOriginPlan(1, PROPOSAL, f.proof))
            rejected(FailureReason.STORAGE_FAILURE, owner.verifyOriginPredecessor(f.original, PROPOSAL, f.proof))
            f.assertReadOnly(before)
        }
    }
    ...
    @Test fun acknowledgedCloseAndExistingReopenRetainOnlyTheExactCurrentLedgerAndOriginalProof() = runBlocking {
        fixture { f ->
            ...
            ok(next.verifyOriginPlan(1, PROPOSAL, f.proof))
            rejected(FailureReason.CONFLICT, next.verifyOriginPredecessor(f.original, PROPOSAL, f.proof))
    ...
    @Test fun failedInitializationBecomesCloseOnlyAndRetainsConnectionAcrossFailedClose() = runBlocking {
        fixture { f ->
            f.prepare(); val owner = f.owner(); f.sql.beforeStep = { error("work-private-initialization-canary") }
            val result = owner.open(); rejected(FailureReason.STORAGE_FAILURE, result)
            assertFalse(result.toString().contains("work-private-initialization-canary")); assertEquals(0, f.sql.closeCalls)
    ...
    @Test fun readinessIsNotExposedUntilSuccessfulCallerHandoffAndCloseOrCancellationWinsTheGap() = runTest {
        for (scenario in listOf("cancel-before", "cancel-handoff", "close-handoff", "failed-close-handoff")) fixture { f ->
            f.prepare(); val dispatcher = WorkHandoffDispatcher(); val owner = f.owner(dispatcher = dispatcher)
            ...
            try {
                if (scenario != "cancel-before") {
                    ...
                    val verifying = async(dispatcher, start = CoroutineStart.UNDISPATCHED) { owner.verifyOriginPlan(1, PROPOSAL, f.proof) }
                    assertTrue(reading.isCompleted); assertTrue(writing.isCompleted); assertTrue(verifying.isCompleted)
                    rejected(FailureReason.STALE_SESSION, reading.await()); rejected(FailureReason.STALE_SESSION, writing.await())
                    rejected(FailureReason.STALE_SESSION, verifying.await()); assertEquals(reads, f.sql.executed); f.noKeyEffects()
                }
    ...
    @Test fun unexpectedVaultFailuresAreSanitizedAndCancellationDoesNotBecomeSuccessOrDestroyReadyOwner() = runBlocking {
        fixture { f ->
            ...
            val result = owner.verifyOriginPlan(1, PROPOSAL, f.proof); rejected(FailureReason.STORAGE_FAILURE, result)
            ...
            assertFailsWith<CancellationException> { owner.verifyOriginPlan(1, PROPOSAL, f.proof) }
            ok(owner.verifyOriginPlan(1, PROPOSAL, f.proof)); same(f.original, record(owner)); f.assertReadOnly(before)
            for (value in listOf(owner.toString(), record(owner).toString(), result.toString()))
                for (secret in listOf(f.ownerTag(), f.ownerKey(), PROPOSAL.copyForCodec().decodeToString(), IDLE.copyForCodec().decodeToString()))
    ...
    private class Fixture {
        private val directory = Files.createTempDirectory("feedme-existing-work-recovery-")
        val file = directory.resolve("state.sqlite")
        val vault = WorkRecoveryVault(); val events = mutableListOf<String>()
        lateinit var sql: WorkRecoveryConnection
        lateinit var original: SessionControlRecord; lateinit var proof: PrivateBytes
        var callbackCalls = 0; var callbackFailures = 0
        private val owners = mutableListOf<ExistingSessionWorkRecoveryStore>()
        private val connections = mutableListOf<WorkRecoveryConnection>()
        suspend fun prepare(payload: PrivateBytes = IDLE) {
            ...
            try {
                ...
                original = record(work); proof = ok(work.signOriginPlan(original, PROPOSAL))
            } finally { ok(work.close()); observe() }
        }
        fun owner(dispatcher: CoroutineDispatcher = Dispatchers.IO): ExistingSessionWorkRecoveryStore {
            val connection = WorkRecoveryConnection(BundledSQLiteDriver().open(file.toString(), SQLITE_OPEN_READWRITE or SQLITE_OPEN_FULLMUTEX), events)
                .also { sql = it; connections += it }
            observe()
            return EncryptedStateDatabase.createWorkRecoveryStore(connection, vault, dispatcher) {
                ...
                if (callbackFailures > 0) { callbackFailures--; error("private-work-close-canary") }
            }.also(owners::add)
        }
        fun observe() {
        ...
        suspend fun replaceOwner() {
            val db = ok(EncryptedStateDatabase.open(BundledSQLiteDriver().open(file.toString()), vault))
            try {
                val old = assertNotNull(ok(db.resume(WORK_SCOPE))); ok(old.eraseScope(WORK_SCOPE))
                val fresh = ok(db.activate(WORK_SCOPE)); ok(fresh.commit(WORK_SCOPE, listOf(StoreMutation.Put(WORK_KEY, null, 1, IDLE))))
    ...
    companion object {
        ...
        private val PROPOSAL = bytes("private-bounded-original-work-proposal")
...
private enum class WorkCommitFault { BEFORE, AFTER }
private class WorkHandoffDispatcher : CoroutineDispatcher() {
...
private class WorkRecoveryConnection(private val native: SQLiteConnection, private val events: MutableList<String>) : SQLiteConnection by native {
    ...
    var commitFault: WorkCommitFault? = null; private var changed = false
    override fun prepare(sql: String): SQLiteStatement {
        ...
        return object : SQLiteStatement by statement {
            override fun step(): Boolean {
                ...
                if (fault == WorkCommitFault.BEFORE) error("Injected before SQLite COMMIT")
                ...
                if (fault == WorkCommitFault.AFTER) error("Injected application receipt loss after SQLite COMMIT")
    ...
    override fun close() {
        closeCalls++; if (closeFailures > 0) { closeFailures--; error("private-work-connection-close-canary") }
        native.close(); closed = true; events += "CONNECTION_CLOSED"
    }
}
private class WorkRecoveryVault : StateVault {
