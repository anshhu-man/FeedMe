...
const parsers = ['android-storage-smoke.mjs', 'verify-session-setup-publication.mjs'].map(name => {
  const source = fs.readFileSync(fileURLToPath(new URL(name, import.meta.url)), 'utf8');
  const declaration = source.match(/function bindingSyncFailureEvidence\(log, identities\) \{[\s\S]*?\n\}/)?.[0];
  assert.ok(declaration, `${name} must retain the pure private-binding evidence parser`);
  return vm.runInNewContext(`(${declaration})`, Object.create(null), {timeout: 1000});
});

const testClass = 'com.feedme.storage.AndroidStateBindingSyncFailureTest';
const expected = [
...
const lines = expected.map(([label, , sqlite]) =>
  `INSTRUMENTATION_RESULT: data_binding_sync_${label}=sqlite=${sqlite};failures=1;receipts=0;vfs=1;delegatedUnlink=${sqlite === 1290 ? 1 : 0}`);
const log = lines.join('\n');
const rejects = (text, successful = identities) => {
  for (const parse of parsers) assert.throws(() => parse(text, successful));
};

test('private-binding smoke and verifier agree on all six source-bound initial and replay VFS results', () => {
  const source = fs.readFileSync(fileURLToPath(new URL(
    '../shared/storage/src/androidInstrumentedTest/kotlin/com/feedme/storage/AndroidStateBindingSyncFailureTest.kt',
  ...
  for (const [label, method, sqlite] of expected) assert.deepEqual(results[0][label], {
    ...
    failures: 1, receipts: 0, vfs: 1, delegatedUnlink: sqlite === 1290 ? 1 : 0,
  });
});

test('private-binding evidence requires both initial binding and replay labels for every sync stage', () => {
  for (let omitted = 0; omitted < lines.length; omitted++)
    rejects(lines.filter((_, index) => index !== omitted).join('\n'));
});

test('private-binding evidence rejects duplicate labels even when the total count is still six', () => {
  rejects([...lines.slice(0, -1), lines[0]].join('\n'));
  rejects(`${log}\n${lines[0]}`);
});

test('private-binding evidence rejects forged or additional result labels', () => {
  rejects(log.replace('bind-journal', 'select-unrecognized'));
  rejects(`${log}\n${lines[0].replace('bind-journal', 'unrecognized-recovery')}`);
});

test('private-binding result bytes cannot substitute for a missing paired-success source identity', () => {
...
test('private-binding evidence rejects successful identities borrowed from another class or method', () => {
  rejects(log, identities.map(identity => identity.replace('AndroidStateBindingSyncFailureTest', 'AndroidSessionLedgerSyncFailureTest')));
  rejects(log, identities.map(identity => `${identity}Unrelated`));
});

test('private-binding evidence rejects an incorrect SQLite extended error for the exact stage', () => {
...
test('private-binding evidence requires one actual failure and zero returned binding receipts', () => {
  for (const replacement of ['failures=0', 'failures=2', 'failures=-1'])
    rejects(log.replace('failures=1', replacement));
  rejects(log.replace('receipts=0', 'receipts=1'));
});

test('private-binding evidence preserves the explicit VFS mechanism and delegated-unlink stage', () => {
...
test('private-binding evidence rejects truncated reordered or extra result fields', () => {
  rejects(log.replace(';receipts=0', ''));
  rejects(log.replace('failures=1;receipts=0', 'receipts=0;failures=1'));
