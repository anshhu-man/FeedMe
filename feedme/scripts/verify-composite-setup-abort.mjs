...
// Source-bound already-open explicit composite abort; startup factories remain a separate gate.
...
const destination = path.join(root, 'docs/verification/composite-setup-abort');
...
const report = {
  ...
  scope: "M1.05d.5b.2b.3.6c already-open explicit composite setup abort: opaque runtime-generation-bound proposal over exact native evidence, fresh acknowledged abortRequested control, ordered work/data/credential cleanup, unchanged-other-resource fences, reacknowledgement of every component on pending retry, and Complete only after exact acknowledgements. A retained same-runtime final-Complete receipt retry cannot be reconstructed from persisted Complete alone. Binding bytes are derived from original canonical session identities and authenticated native target; optional native credential abort authenticates scope and plan within the same parent mutex. Real-provider/confirmation UI, existing-only startup factories, failed-open/close ownership, integrated process-separation matrix, physical devices/power loss, public hot-journal recovery, API26/iOS and release acceptance remain open. All previous native tests,39 VFS labels/five controlled process stages and isolated four-ABI helper checks remain. App acknowledgement-loss wrappers are not SQLite VFS or OS power-loss faults. The demo and public repository snapshot are unchanged; no deployment or publication.",
...
try {
  ...
  for (const [directory, name, expected] of [
    ...
    ['shared/storage/build/test-results/jvmTest', 'storage', 466],
